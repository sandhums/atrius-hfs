//! **Profile** validation using canonical examples from `crates/fhir/tests/data/json/R5/`
//! combined with the Atrius test profile in `tests/fixtures/r5/profile/`.
//!
//! The published FHIR example bundle does not declare `meta.profile`; tests inject it so we
//! exercise `validate_resource_with_profiles` on real-world shapes.
...
use crate::common::fhir_json_examples::{load_r5_fhir_resource, read_fhir_example_json};
