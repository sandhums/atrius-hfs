...
//!
//! Example codes indicating the reason why an observation is in its current status. Note that these are in no way complete and might not even be appropriate for some uses.
use crate::TerminologyValidationError;
use helios_fhir::r6::{CodeableConcept, Coding};

pub struct ObservationStatusReason;

impl ObservationStatusReason {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/observation-status-reason";
    pub const VERSION: &'static str = "6.0.0-ballot4";
    pub const MODULE: &'static str = "observation_status_reason_13976e67";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/CodeSystem/observation-status-reason" {
            return Some(
                super::super::code_systems::ObservationStatusReasonCodes::try_from_code(code)
                    .is_some(),
            );
        }
        None
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/CodeSystem/observation-status-reason" {
            return super::super::code_systems::ObservationStatusReasonCodes::try_from_code(code)
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            ...
            Some(false) => Err(TerminologyValidationError::NotInValueSet {
                valueset_url: Self::URL.to_string(),
                system: Some(
                    "http://hl7.org/fhir/CodeSystem/observation-status-reason".to_string(),
                ),
                code: code.to_string(),
            }),
            None => Err(TerminologyValidationError::RemoteValidationRequired(
