...
pub struct ConditionStageType;

impl ConditionStageType {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/condition-stage-type";
    pub const VERSION: &'static str = "6.0.0-ballot4";
    pub const MODULE: &'static str = "condition_stage_type";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://snomed.info/sct" {
            if matches!(code, "261023001") {
                return Some(true);
            }
            return None;
        }
        if system == "http://snomed.info/sct" {
            if matches!(code, "260998006") {
                return Some(true);
            }
            return None;
        }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://snomed.info/sct" {
            return match code {
                "261023001" => Some("Pathological staging (qualifier value)"),
                _ => None,
            };
        }
        if system == "http://snomed.info/sct" {
            return match code {
                "260998006" => Some("Clinical staging (qualifier value)"),
                _ => None,
            };
        }
        None
