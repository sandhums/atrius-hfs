...
//! Diagnostic Report Status Reason
//!
//! Example codes indicating the reason why a diagnostic report is in its current status. Note that these are in no way complete and might not even be appropriate for some uses.
use crate::TerminologyValidationError;
use helios_fhir::r6::{CodeableConcept, Coding};

pub struct DiagnosticReportStatusReason;

impl DiagnosticReportStatusReason {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/diagnostic-report-status-reason";
    pub const VERSION: &'static str = "6.0.0-ballot4";
    pub const MODULE: &'static str = "diagnostic_report_status_reason_f0d2b095";
    ...
    pub const INCLUDED_SYSTEMS: &'static [&'static str] =
        &["http://hl7.org/fhir/CodeSystem/diagnostic-report-status-reason"];
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/CodeSystem/diagnostic-report-status-reason" {
            return Some(
                super::super::code_systems::DiagnosticReportStatusReasonCodes::try_from_code(code)
                    .is_some(),
            );
        }
        None
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/CodeSystem/diagnostic-report-status-reason" {
            return super::super::code_systems::DiagnosticReportStatusReasonCodes::try_from_code(
                code,
            )
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            ...
            Some(false) => Err(TerminologyValidationError::NotInValueSet {
                valueset_url: Self::URL.to_string(),
                system: Some(
                    "http://hl7.org/fhir/CodeSystem/diagnostic-report-status-reason".to_string(),
                ),
                code: code.to_string(),
            }),
            None => Err(TerminologyValidationError::RemoteValidationRequired(
