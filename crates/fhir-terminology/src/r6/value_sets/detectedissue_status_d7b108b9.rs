...
pub struct DetectedIssueStatus;

impl DetectedIssueStatus {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/detectedissue-status";
    pub const VERSION: &'static str = "6.0.0-ballot4";
    pub const MODULE: &'static str = "detectedissue_status_d7b108b9";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/observation-status" {
            return Some(matches!(
                code,
                "preliminary" | "final" | "entered-in-error" | "unknown"
            ));
        }
        if system == "http://hl7.org/fhir/detectedissue-status" {
            return Some(matches!(code, "mitigated"));
        }
        None
    }
