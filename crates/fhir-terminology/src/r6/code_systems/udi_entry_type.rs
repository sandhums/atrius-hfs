// @generated by fhir-valueset-gen
// DO NOT EDIT MANUALLY

//! UDI Entry Type
//!
//! Codes to identify how UDI data was entered.

use helios_fhir::r6::{CodeableConcept, Coding};

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[allow(non_camel_case_types)]
pub enum UDIEntryType {
    Barcode,
    Rfid,
    Manual,
    Card,
    SelfReported,
    ElectronicTransmission,
    Unknown,
}

impl UDIEntryType {
    pub const URL: &'static str = "http://hl7.org/fhir/udi-entry-type";
    pub const VERSION: &'static str = "6.0.0-ballot4";
    pub const STATUS: &'static str = "active";
    pub const CONTENT: &'static str = "complete";
    pub const EXPERIMENTAL: bool = false;
    pub const CASE_SENSITIVE: bool = true;
    pub const HAS_CONCEPTS: bool = true;

    pub fn as_code(self) -> &'static str {
        match self {
            Self::Barcode => "barcode",
            Self::Rfid => "rfid",
            Self::Manual => "manual",
            Self::Card => "card",
            Self::SelfReported => "self-reported",
            Self::ElectronicTransmission => "electronic-transmission",
            Self::Unknown => "unknown",
        }
    }

    pub fn try_from_code(code: &str) -> Option<Self> {
        Some(match code {
            "barcode" => Self::Barcode,
            "rfid" => Self::Rfid,
            "manual" => Self::Manual,
            "card" => Self::Card,
            "self-reported" => Self::SelfReported,
            "electronic-transmission" => Self::ElectronicTransmission,
            "unknown" => Self::Unknown,
            _ => return None,
        })
    }

    pub fn display(self) -> Option<&'static str> {
        match self {
            Self::Barcode => Some("Barcode"),
            Self::Rfid => Some("RFID"),
            Self::Manual => Some("Manual"),
            Self::Card => Some("Card"),
            Self::SelfReported => Some("Self Reported"),
            Self::ElectronicTransmission => Some("Electronic Transmission"),
            Self::Unknown => Some("Unknown"),
        }
    }

    pub fn definition(self) -> Option<&'static str> {
        match self {
            Self::Barcode => Some("a barcodescanner captured the data from the device label."),
            Self::Rfid => Some("An RFID chip reader captured the data from the device label."),
            Self::Manual => Some(
                "The data was read from the label by a person and manually entered. (e.g.  via a keyboard).",
            ),
            Self::Card => Some(
                "The data originated from a patient\'s implant card and was read by an operator.",
            ),
            Self::SelfReported => Some(
                "The data originated from a patient source and was not directly scanned or read from a label or card.",
            ),
            Self::ElectronicTransmission => Some(
                "The UDI information was received electronically from the device through a communication protocol, such as the IEEE 11073 20601 version 4 exchange protocol over Bluetooth or USB.",
            ),
            Self::Unknown => Some("The method of data capture has not been determined."),
        }
    }

    pub fn parent_code(self) -> Option<&'static str> {
        match self {
            Self::Barcode => None,
            Self::Rfid => None,
            Self::Manual => None,
            Self::Card => None,
            Self::SelfReported => None,
            Self::ElectronicTransmission => None,
            Self::Unknown => None,
        }
    }

    pub fn level(self) -> u16 {
        match self {
            Self::Barcode => 0,
            Self::Rfid => 0,
            Self::Manual => 0,
            Self::Card => 0,
            Self::SelfReported => 0,
            Self::ElectronicTransmission => 0,
            Self::Unknown => 0,
        }
    }
    pub fn to_coding(self) -> Coding {
        Coding {
            id: None,
            extension: None,
            system: Some(Self::URL.to_string().into()),
            version: Some(Self::VERSION.to_string().into()),
            code: Some(self.as_code().to_string().into()),
            display: self.display().map(|d| d.to_string().into()),
            user_selected: None,
        }
    }

    pub fn to_codeable_concept(self) -> CodeableConcept {
        let coding = self.to_coding();
        CodeableConcept {
            id: None,
            extension: None,
            coding: Some(vec![coding]),
            text: self.display().map(|d| d.to_string().into()),
        }
    }

    pub fn matches_coding(self, coding: &Coding) -> bool {
        let system = coding.system.as_ref().and_then(|e| e.value.as_deref());
        let code = coding.code.as_ref().and_then(|e| e.value.as_deref());
        system == Some(Self::URL) && code == Some(self.as_code())
    }

    pub fn matches_codeable_concept(self, cc: &CodeableConcept) -> bool {
        cc.coding
            .as_ref()
            .map(|v| {
                v.iter().any(|c| {
                    let system = c.system.as_ref().and_then(|e| e.value.as_deref());
                    let code = c.code.as_ref().and_then(|e| e.value.as_deref());
                    system == Some(Self::URL) && code == Some(self.as_code())
                })
            })
            .unwrap_or(false)
    }
}
