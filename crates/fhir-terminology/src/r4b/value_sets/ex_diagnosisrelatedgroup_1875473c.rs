...
impl ExampleDiagnosisRelatedGroupCodes {
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/ex-diagnosisrelatedgroup" {
            return Some(
                super::super::code_systems::ExampleDiagnosisRelatedGroupCodes::try_from_code(code)
                    .is_some(),
            );
        }
        None
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/ex-diagnosisrelatedgroup" {
            return super::super::code_systems::ExampleDiagnosisRelatedGroupCodes::try_from_code(
                code,
            )
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            ...
            Some(false) => Err(TerminologyValidationError::NotInValueSet {
                valueset_url: Self::URL.to_string(),
                system: Some(
                    "http://terminology.hl7.org/CodeSystem/ex-diagnosisrelatedgroup".to_string(),
                ),
                code: code.to_string(),
            }),
            None => Err(TerminologyValidationError::RemoteValidationRequired(
