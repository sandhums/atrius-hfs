...
impl SubscriptionManager {
    ...
    pub fn register(
        ...
        // Status write-back must not wipe consecutive_failures either —
        // otherwise a feedback outbox event after persist_status would
        // permanently stall error/off.
        let key = (tenant_id.to_string(), subscription_id.to_string());
        let previous = self.subscriptions.get(&key).map(|entry| {
            let sub = entry.value();
            (
                sub.status,
                sub.events_since_start,
                sub.consecutive_failures,
                sub.last_notification_at,
            )
        });

        let (events_since_start, consecutive_failures, last_notification_at) = match previous {
            Some((previous_status, events, failures, last_at))
                if !is_rearm(previous_status, status) =>
            {
                (events, failures, last_at)
            }
            _ => (0, 0, chrono::Utc::now()),
        };
