...
impl SequenceOntology {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/sequenceontology";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "sequenceontology";
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://www.sequenceontology.org".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://www.sequenceontology.org", code)
    }
