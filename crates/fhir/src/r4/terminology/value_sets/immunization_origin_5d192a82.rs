...
pub struct ImmunizationOriginCodes;

impl ImmunizationOriginCodes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/immunization-origin";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "immunization_origin_5d192a82";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/immunization-origin" {
            return Some(matches!(code, "provider" | "record" | "recall" | "school"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/immunization-origin" {
            return match code {
                ...
                "record" => Some("Written Record"),
                "recall" => Some("Parent/Guardian/Patient Recall"),
                "school" => Some("School Record"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/immunization-origin".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/immunization-origin", code)
    }
