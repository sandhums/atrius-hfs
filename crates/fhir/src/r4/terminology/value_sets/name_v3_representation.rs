...
pub struct NameRepresentationUse;

impl NameRepresentationUse {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/name-v3-representation";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "name_v3_representation";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-EntityNameUse" {
            return Some(matches!(code, "ABC" | "IDE" | "SYL"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-EntityNameUse".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-EntityNameUse", code)
    }
