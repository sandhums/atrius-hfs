...
pub struct ICD10Codes;

impl ICD10Codes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/icd-10";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "icd_10";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/sid/icd-10" {
            return Some(matches!(code, "123456" | "123457" | "987654" | "123987" | "112233" | "997755" | "321789"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/sid/icd-10".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/sid/icd-10", code)
    }
