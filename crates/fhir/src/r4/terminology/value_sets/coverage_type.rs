...
pub struct CoverageTypeAndSelfPayCodes;

impl CoverageTypeAndSelfPayCodes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/coverage-type";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "coverage_type";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/coverage-selfpay" {
            return Some(super::super::code_systems::CoverageSelfPayCodes::try_from_code(code).is_some());
        }

        // Delegation to included ValueSets (compose.include.valueSet)
        let mut any_none = false;
        let mut any_false = false;
        // http://terminology.hl7.org/ValueSet/v3-ActCoverageTypeCode (not generated locally)
        any_none = true;
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/coverage-selfpay" {
            return super::super::code_systems::CoverageSelfPayCodes::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/coverage-selfpay".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/coverage-selfpay", code)
    }
