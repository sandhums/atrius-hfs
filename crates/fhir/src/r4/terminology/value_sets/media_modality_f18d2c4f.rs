...
impl MediaModality {
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/media-modality" {
            return Some(super::super::code_systems::MediaModality::try_from_code(code).is_some());
        }

        // Delegation to included ValueSets (compose.include.valueSet)
        let mut any_none = false;
        let mut any_false = false;
        // http://hl7.org/fhir/ValueSet/dicom-cid29 (not generated locally)
        any_none = true;
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/media-modality" {
            return super::super::code_systems::MediaModality::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
