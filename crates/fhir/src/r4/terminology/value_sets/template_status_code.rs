...
impl TemplateStatusCode {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/template-status-code";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "template_status_code";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "urn:oid:2.16.840.1.113883.3.1937.98.5.8" {
            return Some(matches!(code, "draft" | "pending" | "active" | "review" | "cancelled" | "rejected" | "retired" | "terminated"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "urn:oid:2.16.840.1.113883.3.1937.98.5.8" {
            return match code {
                "draft" => Some("Draft"),
                "pending" => Some("Under pre-publication review"),
                "active" => Some("Active"),
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("urn:oid:2.16.840.1.113883.3.1937.98.5.8".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("urn:oid:2.16.840.1.113883.3.1937.98.5.8", code)
    }
