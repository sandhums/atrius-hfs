...
impl ProvenanceActivityType {
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-DocumentCompletion" {
            return Some(matches!(code, "LA"));
        }
        if system == "http://terminology.hl7.org/CodeSystem/v3-ActCode" {
            return Some(matches!(code, "ANONY" | "DEID" | "MASK" | "LABEL" | "PSEUD"));
        }
        if system == "http://terminology.hl7.org/CodeSystem/v3-DataOperation" {
            return Some(matches!(code, "CREATE" | "DELETE" | "UPDATE" | "APPEND" | "NULLIFY"));
        }
        None
    }
