...
impl PostalAddressUse {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/postal-address-use";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "postal_address_use";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-AddressUse" {
            return Some(matches!(code, "BAD" | "CONF" | "HP" | "HV" | "DIR" | "PUB" | "PHYS" | "PST"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-AddressUse" {
            return match code {
                "BAD" => Some("bad address"),
                "CONF" => Some("confidential address"),
                "HP" => Some("primary home"),
                "HV" => Some("vacation home"),
                "DIR" => Some("direct"),
                "PUB" => Some("public"),
                "PHYS" => Some("physical visit address"),
                "PST" => Some("postal address"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-AddressUse".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-AddressUse", code)
    }
