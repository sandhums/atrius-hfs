...
pub struct DocumentReferenceFormatCodeSet;

impl DocumentReferenceFormatCodeSet {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/formatcodes";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "formatcodes";
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://ihe.net/fhir/ValueSet/IHE.FormatCode.codesystem".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://ihe.net/fhir/ValueSet/IHE.FormatCode.codesystem", code)
    }
