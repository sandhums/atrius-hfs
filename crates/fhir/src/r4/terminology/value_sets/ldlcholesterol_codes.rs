...
impl LDLCodes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/ldlcholesterol-codes";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "ldlcholesterol_codes";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://loinc.org" {
            return Some(matches!(code, "18262-6" | "13457-7"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://loinc.org" {
            return match code {
                "18262-6" => Some("LDL Cholesterol (Assay)"),
                "13457-7" => Some("Cholesterol in LDL [Mass/volume] in Serum or Plasma by calculation"),
                _ => None,
            };
        }
        None
