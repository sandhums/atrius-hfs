...
impl ProbabilityDistributionType {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/probability-distribution-type";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "probability_distribution_type";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-ProbabilityDistributionType" {
            return Some(matches!(code, "B" | "E" | "F" | "G" | "LN" | "N" | "T" | "U" | "X2"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-ProbabilityDistributionType".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-ProbabilityDistributionType", code)
    }
