...
pub struct ProvenanceHistoryRecordActivityCodes;

impl ProvenanceHistoryRecordActivityCodes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/provenance-history-record-activity";
    pub const VERSION: &'static str = "4.0.1";
    pub const MODULE: &'static str = "provenance_history_record_activity";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-DataOperation" {
            return Some(matches!(code, "CREATE" | "UPDATE" | "DELETE" | "ABORT" | "HOLD" | "RELEASE" | "CANCEL" | "ACTIVATE" | "SUSPEND" | "RESUME" | "COMPLETE" | "NULLIFY" | "OBSOLETE" | "REACTIVATE"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-DataOperation" {
            return match code {
                ...
                "UPDATE" => Some("Updated"),
                "DELETE" => Some("Deleted"),
                "ABORT" => Some("Stopped/Ended/Aborted"),
                "HOLD" => Some("Held"),
                "RELEASE" => Some("Released"),
                "CANCEL" => Some("Cancelled"),
                "ACTIVATE" => Some("Activated"),
                "SUSPEND" => Some("Suspended"),
                "RESUME" => None,
                "COMPLETE" => Some("Completed"),
                "NULLIFY" => Some("Mark Entered-in-error"),
                "OBSOLETE" => Some("Replaced"),
                "REACTIVATE" => Some("Reactivated"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-DataOperation".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-DataOperation", code)
    }
