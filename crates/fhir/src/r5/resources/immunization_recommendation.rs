// @generated by helios-fhir-gen (R5 ImmunizationRecommendation) — do not edit
...
/// FHIR ImmunizationRecommendation type
/// 
...
/// FHIR Version: 5.0.0
/// 
/// See: [ImmunizationRecommendation](http://hl7.org/fhir/StructureDefinition/ImmunizationRecommendation)
#[derive(Debug, Clone, PartialEq, FhirSerde, FhirPath, Default)]
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,patient,date,recommendation")]
pub struct ImmunizationRecommendation {
    ...
    /// 
    /// ## Implementation Notes
    /// A given instance of the .recommendation backbone element should correspond to
    /// a single recommended administration.
    /// 
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **imr-1**: One of vaccineCode or targetDisease SHALL be present (error)
...
pub struct ImmunizationRecommendationRecommendation {
    ...
    /// ## Implementation Notes
    /// A given instance of the .recommendation backbone element should correspond to
    /// a single recommended administration but sometimes that administration (e.g.
    /// MMR) will encompass multiple diseases, thus the targetDisease element is
    /// allowed to repeat.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    ...
    /// 
    /// ## Special Semantics
    /// - Modifier element - This element is labeled as a modifier because the forecastStatus indicates whether a patient needs additional doses (due, overdue, etc.) or not  (contraindicated, complete, etc.).
    /// - Included in summary
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
