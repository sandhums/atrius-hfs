// @generated by helios-fhir-gen (R5 ServiceRequest) — do not edit
...
#[fhir_resource(
    ...
    summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,instantiates_canonical,instantiates_uri,based_on,replaces,requisition,status,intent,category,priority,do_not_perform,code,order_detail,quantity,subject,focus,encounter,occurrence,as_needed,authored_on,requester,performer_type,performer,location,reason,specimen,body_site,body_structure"
)]
pub struct ServiceRequest {
    ...
    ///
    /// ## Implementation Notes
    /// The PlanDefinition resource is used to describe series, sequences, or groups
    /// of actions to be taken, while the ActivityDefinition resource is used to
    /// define each specific step or activity to be performed. More information can
    /// be found in the [Boundaries and Relationships](plandefinition.html#12.23.2)
    /// section for PlanDefinition.
    ///
    /// ## Cardinality: Optional, Multiple (0..*)
    ///
    /// ## Special Semantics
    ...
    /// *Condition* should be used if available. Otherwise, use `concept.text`
    /// element if the data is free (uncoded) text as shown in the [CT Scan
    ...
    ///
    /// ## Implementation Notes
    /// The purpose of the ServiceRequest.specimen is to reflect the actual specimen
    /// that the requested test/procedure is asked to be performed on, whether the
    /// lab already has it or not. References to specimens for purposes other than to
    /// perform a test/procedure on should be made using the
    /// ServiceRequest.supportingInfo or the Specimen.parent where the
    /// Specimen.parent would enable descendency and ServiceRequest.supportingInfo a
