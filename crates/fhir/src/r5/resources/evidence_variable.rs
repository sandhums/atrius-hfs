// @generated by helios-fhir-gen (R5 EvidenceVariable) — do not edit
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,identifier,version,version_algorithm,name,title,short_title,status,date,publisher,contact,description,use_context,characteristic")]
pub struct EvidenceVariable {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **evv-1**: In a characteristic, at most one of these six elements shall be used: definitionReference or definitionCanonical or definitionCodeableConcept or definitionId or definitionByTypeAndValue or definitionByCombination (error)
...
///   Expression: `(definitionReference.count() + definitionCanonical.count() + definitionCodeableConcept.count() + definitionId.count() + definitionByTypeAndValue.count() + definitionByCombination.count())  < 2`
#[derive(Debug, Clone, PartialEq, FhirSerde, FhirPath, Default)]
#[fhir_resource(choice_elements = "instances,duration")]
pub struct EvidenceVariableCharacteristic {
    ...
    /// matching this characteristic definition is equivalent to meeting this
    /// criterion.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// 
    /// ## Implementation Notes
    /// When another element provides a definition of the characteristic, the
    /// definitionExpression content SHALL match the definition (only adding
    /// technical concepts necessary for implementation) without changing the
    /// meaning.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
...
pub struct EvidenceVariableCharacteristicDefinitionByCombination {
    ...
    /// 
    /// ## Requirements
    /// If code is "at-least" or "at-most" then threshold SHALL be used. If code is
    /// neither "at-least" nor "at-most" then threshold SHALL NOT be used.
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    /// ## Constraints
