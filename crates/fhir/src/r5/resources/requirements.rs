// @generated by helios-fhir-gen (R5 Requirements) — do not edit
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,identifier,version,version_algorithm,name,title,status,experimental,date,publisher,contact,use_context,jurisdiction,derived_from")]
pub struct Requirements {
    ...
    /// 
    /// A legal or geographic region in which the Requirements is intended to be
    /// used.
    /// 
    /// ## Implementation Notes
    ...
    /// describe in narrative if they don't apply to all actors.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
...
pub struct RequirementsStatement {
    ...
    /// 
    /// ## Implementation Notes
    /// This can be just a key, if there is only one matching statement in the
    /// requirements statements referenced in Requirements.derivedFrom. If there is
    /// more than one, it should be in the format ```url#key```
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// 
    /// ## Implementation Notes
    /// Follows the same convention as statement.derivedFrom, where a reference is
    /// the canonical of the Requirements followed by '#' and the 'key' of the
    ...
    /// rule that's met by the base FHIR spec, or a national implementation guide.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
