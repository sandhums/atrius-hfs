// @generated by helios-fhir-gen (R5 Provenance) — do not edit
...
#[fhir_resource(choice_elements = "occurred", summary_fields = "id,meta,implicit_rules,modifier_extension,target,recorded,agent,entity")]
pub struct Provenance {
    ...
    /// set of resources being maintained (i.e. a document). When using the RESTful
    /// API, the identity of the resource might not be known (especially not the
    ...
    /// 
    /// ## Implementation Notes
    /// This can be a little different from the time stamp on the resource if there
    /// is a delay between recording the event and updating the provenance and target
    /// resource.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// created/updated (.target) by the activity does not involve a subject. Note
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **prov-1**: Who and onBehalfOf cannot be the same (error)
    ///   Expression: `who.resolve().exists() and onBehalfOf.resolve().exists() implies who.resolve() != onBehalfOf.resolve()`
    /// - **prov-2**: If who is a PractitionerRole, onBehalfOf can't reference the same Practitioner (error)
    ...
    /// - **prov-3**: If who is an organization, onBehalfOf can't be a PractitionerRole within that organization (error)
...
pub struct ProvenanceAgent {
    ...
    /// 
    /// ## Conditions
    /// Used when: prov-1, prov-2, prov-3
    pub who: Reference,
...
pub struct ProvenanceEntity {
    ...
    /// same custodianship as the Target, and thus the meaning of
    /// Provenance.entity.agent is to say that the entity referenced is managed
