...
pub struct ValueFilterComparator;

impl ValueFilterComparator {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/value-filter-comparator";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "value_filter_comparator";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/search-comparator" {
            return Some(matches!(code, "eq" | "gt" | "lt" | "ge" | "le" | "sa" | "eb"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/search-comparator" {
            return match code {
                "eq" => Some("Equals"),
                "gt" => Some("Greater Than"),
                "lt" => Some("Less Than"),
                "ge" => Some("Greater or Equals"),
                "le" => Some("Less of Equal"),
                "sa" => Some("Starts After"),
                "eb" => Some("Ends Before"),
                _ => None,
            };
        }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/search-comparator".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/search-comparator", code)
    }
