...
pub struct LegalStatusOfSupply;

impl LegalStatusOfSupply {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/legal-status-of-supply";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "legal_status_of_supply_0a2f695b";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/legal-status-of-supply" {
            return Some(super::super::code_systems::LegalStatusOfSupply::try_from_code(code).is_some());
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/legal-status-of-supply" {
            return super::super::code_systems::LegalStatusOfSupply::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/legal-status-of-supply".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/legal-status-of-supply", code)
    }
