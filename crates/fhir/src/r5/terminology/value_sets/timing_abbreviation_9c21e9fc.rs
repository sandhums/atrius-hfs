...
impl TimingAbbreviation {
    ...
    pub const MODULE: &'static str = "timing_abbreviation_9c21e9fc";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/timing-abbreviation" {
            return Some(super::super::code_systems::TimingAbbreviation::try_from_code(code).is_some());
        }
        if system == "http://terminology.hl7.org/CodeSystem/v3-GTSAbbreviation" {
            return Some(matches!(code, "BID" | "TID" | "QID" | "AM" | "PM" | "QD" | "QOD" | "Q1H" | "Q2H" | "Q3H" | "Q4H" | "Q6H" | "Q8H" | "BED" | "WK" | "MO"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/timing-abbreviation" {
            return super::super::code_systems::TimingAbbreviation::try_from_code(code).and_then(|c| c.display());
        }
        if system == "http://terminology.hl7.org/CodeSystem/v3-GTSAbbreviation" {
            return match code {
                "BID" => Some("BID"),
                ...
                "QOD" => Some("QOD"),
                "Q1H" => Some("every hour"),
                "Q2H" => Some("every 2 hours"),
                "Q3H" => Some("every 3 hours"),
                "Q4H" => Some("Q4H"),
                "Q6H" => Some("Q6H"),
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/timing-abbreviation".to_string()), code: code.to_string() })
            }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/timing-abbreviation", code)
    }
