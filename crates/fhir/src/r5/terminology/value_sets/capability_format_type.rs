...
pub struct CapabilityFormatType;

impl CapabilityFormatType {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/capability-format-type";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "capability_format_type";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/fhir-format-type" {
            return Some(super::super::code_systems::FHIRFormatType::try_from_code(code).is_some());
        }
        if system == "urn:ietf:bcp:13" {
            return Some(matches!(code, "application/fhir+json" | "application/fhir+xml" | "application/fhir+ttl"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/fhir-format-type" {
            return super::super::code_systems::FHIRFormatType::try_from_code(code).and_then(|c| c.display());
        }
        if system == "urn:ietf:bcp:13" {
            return match code {
                "application/fhir+json" => None,
                "application/fhir+xml" => None,
                "application/fhir+ttl" => None,
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/fhir-format-type".to_string()), code: code.to_string() })
            }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/fhir-format-type", code)
    }
