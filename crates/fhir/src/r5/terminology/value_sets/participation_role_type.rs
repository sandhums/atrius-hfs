...
pub struct ParticipationRoleType;

impl ParticipationRoleType {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/participation-role-type";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "participation_role_type";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        ...
        // http://terminology.hl7.org/ValueSet/provenance-agent-type (not generated locally)
        any_none = true;
        // http://dicom.nema.org/medical/dicom/current/output/chtml/part16/sect_CID_402.html (not generated locally)
        any_none = true;
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/extra-security-role-type".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/extra-security-role-type", code)
    }
