...
pub struct AppointmentResponseStatus;

impl AppointmentResponseStatus {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/appointmentresponse-status";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "appointmentresponse_status";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/participationstatus" {
            return Some(super::super::code_systems::ParticipationStatus::try_from_code(code).is_some());
        }
        if system == "http://hl7.org/fhir/appointmentstatus" {
            return Some(matches!(code, "entered-in-error"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/participationstatus" {
            return super::super::code_systems::ParticipationStatus::try_from_code(code).and_then(|c| c.display());
        }
        if system == "http://hl7.org/fhir/appointmentstatus" {
            return match code {
                "entered-in-error" => Some("Entered in error"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/participationstatus".to_string()), code: code.to_string() })
            }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/participationstatus", code)
    }
