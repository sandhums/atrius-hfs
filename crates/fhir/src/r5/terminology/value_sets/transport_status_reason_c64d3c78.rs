...
pub struct TransportStatusReason;

impl TransportStatusReason {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/transport-status-reason";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "transport_status_reason_c64d3c78";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/transport-status-reason" {
            return Some(super::super::code_systems::TransportStatus_6f51b4ee::try_from_code(code).is_some());
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/transport-status-reason" {
            return super::super::code_systems::TransportStatus_6f51b4ee::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/transport-status-reason".to_string()), code: code.to_string() })
            }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/transport-status-reason", code)
    }
