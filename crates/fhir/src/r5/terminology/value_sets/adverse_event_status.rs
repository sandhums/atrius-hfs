...
pub struct AdverseEventStatus;

impl AdverseEventStatus {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/adverse-event-status";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "adverse_event_status";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/event-status" {
            return Some(matches!(code, "in-progress" | "completed" | "entered-in-error" | "unknown"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/event-status".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
