...
impl EventTiming {
    ...
    pub const MODULE: &'static str = "event_timing_59683b06";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/event-timing" {
            return Some(super::super::code_systems::EventTiming::try_from_code(code).is_some());
        }
        if system == "http://terminology.hl7.org/CodeSystem/v3-TimingEvent" {
            return Some(matches!(code, "HS" | "WAKE" | "C" | "CM" | "CD" | "CV" | "AC" | "ACM" | "ACD" | "ACV" | "PC" | "PCM" | "PCD" | "PCV"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/event-timing" {
            return super::super::code_systems::EventTiming::try_from_code(code).and_then(|c| c.display());
        }
        if system == "http://terminology.hl7.org/CodeSystem/v3-TimingEvent" {
            return match code {
                "HS" => None,
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/event-timing".to_string()), code: code.to_string() })
            }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/event-timing", code)
    }
