...
pub struct EvidenceVariableEvent;

impl EvidenceVariableEvent {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/evidence-variable-event";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "evidence_variable_event_62c00614";
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/evidence-variable-event".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/evidence-variable-event", code)
    }
