...
impl EndpointPayloadType {
    ...
    pub const MODULE: &'static str = "endpoint_payload_type";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        ...
        // http://terminology.hl7.org/ValueSet/v3-HL7FormatCodes (not generated locally)
        any_none = true;
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/endpoint-payload-type".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/endpoint-payload-type", code)
    }
