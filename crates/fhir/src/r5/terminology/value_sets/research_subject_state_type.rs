...
pub struct ResearchSubjectStateType;

impl ResearchSubjectStateType {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/research-subject-state-type";
    pub const VERSION: &'static str = "5.0.0";
    pub const MODULE: &'static str = "research_subject_state_type";
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/research-subject-state-type".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/research-subject-state-type", code)
    }
