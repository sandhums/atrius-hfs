// @generated by fhir-valueset-gen
// DO NOT EDIT MANUALLY

//! Naming System Identifier Type
//!
//! Identifies the style of unique identifier used to identify a namespace.

use super::super::super::{CodeableConcept, Coding};

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[allow(non_camel_case_types)]
pub enum NamingSystemIdentifierType {
    Oid,
    Uuid,
    Uri,
    IriStem,
    V2csmnemonic,
    Other,
}

impl NamingSystemIdentifierType {
    pub const URL: &'static str = "http://hl7.org/fhir/namingsystem-identifier-type";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const STATUS: &'static str = "active";
    pub const CONTENT: &'static str = "complete";
    pub const EXPERIMENTAL: bool = false;
    pub const CASE_SENSITIVE: bool = true;
    pub const HAS_CONCEPTS: bool = true;

    pub fn as_code(self) -> &'static str {
        match self {
            Self::Oid => "oid",
            Self::Uuid => "uuid",
            Self::Uri => "uri",
            Self::IriStem => "iri-stem",
            Self::V2csmnemonic => "v2csmnemonic",
            Self::Other => "other",
        }
    }

    pub fn try_from_code(code: &str) -> Option<Self> {
        Some(match code {
            "oid" => Self::Oid,
            "uuid" => Self::Uuid,
            "uri" => Self::Uri,
            "iri-stem" => Self::IriStem,
            "v2csmnemonic" => Self::V2csmnemonic,
            "other" => Self::Other,
            _ => return None,
        })
    }

    pub fn display(self) -> Option<&'static str> {
        match self {
            Self::Oid => Some("OID"),
            Self::Uuid => Some("UUID"),
            Self::Uri => Some("URI"),
            Self::IriStem => Some("IRI stem"),
            Self::V2csmnemonic => Some("V2CSMNemonic"),
            Self::Other => Some("Other"),
        }
    }

    pub fn definition(self) -> Option<&'static str> {
        match self {
            Self::Oid => Some("An ISO object identifier; e.g. 1.2.3.4.5."),
            Self::Uuid => Some("A universally unique identifier of the form a5afddf4-e880-459b-876e-e4591b0acc11."),
            Self::Uri => Some("A uniform resource identifier (ideally a URL - uniform resource locator); e.g. http://unitsofmeasure.org."),
            Self::IriStem => Some("An IRI string that can be prepended to the code to obtain a concept IRI for RDF applications. This should be a valid, absolute IRI as defined in RFC 3987. See [the RDF spec](rdf.html#iri-stem) for details on how this value may be used."),
            Self::V2csmnemonic => Some("A short string published by HL7 for use in the V2 family of standsrds to idenfify a code system in the V12 coded data types CWE, CNE, and CF. The code values are also published by HL7 at http://www.hl7.org/Special/committees/vocab/table_0396/index.cfm"),
            Self::Other => Some("Some other type of unique identifier; e.g. HL7-assigned reserved string such as LN for LOINC."),
        }
    }

    pub fn parent_code(self) -> Option<&'static str> {
        match self {
            Self::Oid => None,
            Self::Uuid => None,
            Self::Uri => None,
            Self::IriStem => None,
            Self::V2csmnemonic => None,
            Self::Other => None,
        }
    }

    pub fn level(self) -> u16 {
        match self {
            Self::Oid => 0,
            Self::Uuid => 0,
            Self::Uri => 0,
            Self::IriStem => 0,
            Self::V2csmnemonic => 0,
            Self::Other => 0,
        }
    }
    pub fn to_coding(self) -> Coding {
        Coding {
            id: None,
            extension: None,
            system: Some(Self::URL.to_string().into()),
            version: Some(Self::VERSION.to_string().into()),
            code: Some(self.as_code().to_string().into()),
            display: self.display().map(|d| d.to_string().into()),
            user_selected: None,
        }
    }

    pub fn to_codeable_concept(self) -> CodeableConcept {
        let coding = self.to_coding();
        CodeableConcept {
            id: None,
            extension: None,
            coding: Some(vec![coding]),
            text: self.display().map(|d| d.to_string().into()),
        }
    }

    pub fn matches_coding(self, coding: &Coding) -> bool {
        let system = coding.system.as_ref().and_then(|e| e.value.as_deref());
        let code = coding.code.as_ref().and_then(|e| e.value.as_deref());
        system == Some(Self::URL) && code == Some(self.as_code())
    }

    pub fn matches_codeable_concept(self, cc: &CodeableConcept) -> bool {
        cc.coding
            .as_ref()
            .map(|v| {
                v.iter().any(|c| {
                    let system = c.system.as_ref().and_then(|e| e.value.as_deref());
                    let code = c.code.as_ref().and_then(|e| e.value.as_deref());
                    system == Some(Self::URL) && code == Some(self.as_code())
                })
            })
            .unwrap_or(false)
    }

}
