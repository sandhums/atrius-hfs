...
pub struct DetectedIssueCategory;

impl DetectedIssueCategory {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/detectedissue-category";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const MODULE: &'static str = "detectedissue_category";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-ActCode" {
            return Some(matches!(code, "FOOD" | "DRUG" | "ALGY" | "DUPTHPY"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-ActCode" {
            return match code {
                "FOOD" => Some("Food Interaction Alert"),
                "DRUG" => Some("Drug Interaction Alert"),
                "ALGY" => Some("Allergy Alert"),
                "DUPTHPY" => Some("Duplicate Therapy Alert"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-ActCode".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-ActCode", code)
    }
