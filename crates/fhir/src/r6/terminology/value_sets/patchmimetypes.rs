...
pub struct PatchMimeTypes;

impl PatchMimeTypes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/patchmimetypes";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const MODULE: &'static str = "patchmimetypes";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "urn:ietf:bcp:13" {
            return Some(matches!(code, "application/fhir+xml" | "application/fhir+json" | "application/fhir+turtle" | "application/json-patch+json" | "application/xml-patch+xml"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "urn:ietf:bcp:13" {
            return match code {
                ...
                "application/fhir+turtle" => None,
                "application/json-patch+json" => None,
                "application/xml-patch+xml" => None,
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("urn:ietf:bcp:13".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("urn:ietf:bcp:13", code)
    }
