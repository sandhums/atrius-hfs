...
pub struct ExampleComparisonGroup;

impl ExampleComparisonGroup {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/example-comparison-group";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const MODULE: &'static str = "example_comparison_group";
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/example-comparison-group".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/example-comparison-group", code)
    }
