...
pub struct ImagingStudySeriesPerformerFunction;

impl ImagingStudySeriesPerformerFunction {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/series-performer-function";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const MODULE: &'static str = "series_performer_function";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-ParticipationType" {
            return Some(matches!(code, "CON" | "VRF" | "PRF" | "SPRF" | "REF" | "DEV" | "LOC"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-ParticipationType".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-ParticipationType", code)
    }
