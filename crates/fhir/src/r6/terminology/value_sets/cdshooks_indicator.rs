...
impl Indicator {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/cdshooks-indicator";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const MODULE: &'static str = "cdshooks_indicator";
    pub const STATUS: &'static str = "retired";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://cds-hooks.hl7.org/CodeSystem/indicator" {
            return Some(super::super::code_systems::Indicator::try_from_code(code).is_some());
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://cds-hooks.hl7.org/CodeSystem/indicator" {
            return super::super::code_systems::Indicator::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://cds-hooks.hl7.org/CodeSystem/indicator".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://cds-hooks.hl7.org/CodeSystem/indicator", code)
    }
