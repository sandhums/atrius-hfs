...
impl GroupCode {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/group-code";
    pub const VERSION: &'static str = "6.0.0-ballot3";
    pub const MODULE: &'static str = "group_code_792a1dcc";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/group-code" {
            return Some(super::super::code_systems::GroupType_a53ddbfe::try_from_code(code).is_some());
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/group-code" {
            return super::super::code_systems::GroupType_a53ddbfe::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/group-code".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/group-code", code)
    }
