// AUTO-GENERATED by atrius-fhir-generator (R6 Reference)
use crate::r6::*;
...
pub struct Reference {
    ...
    /// flexible approach suitable for use when trading across closed eco-system
    /// boundaries. Absolute URLs do not need to point to a FHIR RESTful server,
    ...
    /// some cases, the type of the target may be determined by inspection of the
    /// reference (e.g. a known RESTful URL) or by resolving the target of the
    /// reference.
    ///
    /// ## Cardinality: Optional (0..1)
    ///
    ...
    /// it understands the business context in which the identifier is used.
    /// Sometimes this is global (e.g. a national identifier) but often it is not.
    /// For this reason, none of the useful mechanisms described for working with
    /// references (e.g. chaining, includes) are possible, nor should servers be
    ...
    /// actual find the target of the reference. The content referred to be the
    /// identifier must meet the logical constraints implied by any limitations on
    /// what resource types are permitted for the reference. For example, it would
    /// not be legitimate to send the identifier for a drug prescription if the type
    ...
    ///
    /// This element only allows for a single identifier. In the case where
    /// additional identifiers are required, use the
    /// [http://hl7.org/fhir/StructureDefinition/additionalIdentifier](https://build.fhir.org/ig/HL7/fhir-extensions/StructureDefinition-additionalIdentifier.html)
    /// extension.
    ///
    /// ## Cardinality: Optional (0..1)
    ///
