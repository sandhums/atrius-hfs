// @generated by helios-fhir-gen (R6 Claim) — do not edit
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,status,status_reason,r#type,r#use,subject,billable_period,created,insurer,provider,priority,insurance")]
pub struct Claim {
    ...
    /// Request authorizing services and products
    /// 
...
pub struct ClaimDiagnosis {
    ...
    /// 
    /// ## Constraints
    /// - **clm-0**: Either a concept or a reference is provided but not both. (error)
...
pub struct ClaimInsurance {
    ...
    /// the future. This is provided so that payers may forward claims to other
    /// payers in the Coordination of Benefit for adjudication rather than the
...
pub struct ClaimProcedure {
    ...
    /// 
    /// ## Constraints
    /// - **clm-1**: Either a concept or a reference is provided but not both. (error)
    ///   Expression: `concept.exists() xor reference.exists()`
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// 
