// @generated by helios-fhir-gen (R6 RiskAssessment) — do not edit
...
#[fhir_resource(choice_elements = "occurrence", summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,status,method,code,subject,encounter,occurrence,condition,performer")]
pub struct RiskAssessment {
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/risk-assessment-method-example
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **ras-2**: Probability as a decimal must be \<= 100 (error)
    ///   Expression: `probability.empty() or ((probability is decimal) implies ((probability as decimal) \<= 100))`
    /// - **ras-3**: Probability as a Quantity must be \<= 100% (error)
    ///   Expression: `probability.empty() or ((probability is Quantity) implies (((probability as Quantity).value \<= 100) and ((probability as Quantity).code = '%') and ((probability as Quantity).system = %ucum)))`
...
pub enum RiskAssessmentPredictionProbability {
    ...
    #[fhir_serde(rename = "probabilityQuantity")]
    Quantity(Quantity),
...
pub struct RiskAssessmentPrediction {
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/risk-assessment-outcome-example
    pub outcome: Option<CodeableConcept>,
    /// Likelihood of specified outcome
    /// 
    /// Indicates how likely the outcome is (in the specified timeframe). Range
    /// SHOULD be expressed as a Quantity (with a unit of '%'), rather than as a
    /// decimal value to make sure the percentage semantics are clear.
    /// 
    /// ## Implementation Notes
