// @generated by helios-fhir-gen (R6 HealthcareService) — do not edit
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,active,provided_by,category,r#type,specialty,location,name,comment,photo")]
pub struct HealthcareService {
    ...
    /// 
    /// ## Implementation Notes
    /// For example, if there is a generic Radiology service that offers CT Scans,
    /// MRIs, etc. You could have HealthcareService resources for the CT Scans and
    /// MRIs, which have an offeredIn reference to the Radiology HealthcareService.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
    ...
    /// HealthcareService itself for specific purposes. E.g. Mailing Addresses,
    /// Billing Addresses, Contact numbers for Booking or Billing Enquiries, general
    /// web address, web address for online bookings etc.
    /// 
    /// If this is empty (or the type of interest is empty), refer to the location's
    /// contacts.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **hcs-1**: To use an eligibility value a code must also be provided to determine the type of eligibility the value applies to. (error)
    ///   Expression: `value.exists() implies code.exists()`
    ...
    /// 
    /// ## Implementation Notes
    /// If more than just a general referral is required then you can combine this
    /// with the eligibility property to specify additional requirements, such as
    /// using the sample code `specialist-referral-required` or other textual details
    /// provided in the eligibility comment.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// service is not available (for things such as public holidays).
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
...
pub struct HealthcareServiceEligibility {
    ...
    /// ## Binding
    /// - **Strength**: example
    /// - **Description**: Coded values under which a specific service is made available.
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/eligibility
    /// 
    /// ## Conditions
    /// Used when: hcs-1
    ...
    /// 
    /// ## Implementation Notes
    /// The example binding could utilize the value where the eligibility code is not
    /// sufficient alone.
    /// 
