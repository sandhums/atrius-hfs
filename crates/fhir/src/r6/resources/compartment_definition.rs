// AUTO-GENERATED by atrius-fhir-generator (R6 CompartmentDefinition)
use crate::r6::*;
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,version,version_algorithm,name,title,status,experimental,date,publisher,contact,use_context,code,search,resource")]
pub struct CompartmentDefinition {
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/compartment-type|6.0.0-ballot3
    pub code: Code,
...
pub struct CompartmentDefinitionResource {
    ...
    /// in more than one way. Note that a given server might use a different 'code'
    /// for the SearchParameter in their CapabilityStatement. The inclusion rules are
    /// based on the 'base' HL7 search parameter code value, not the server-specific
    /// code value. E.g. if a server uses code 'doc' instead of 'performer' for their
    /// Procedure performer search performer, the 'doc' search would still apply when
    /// determining membership in the Practitioner compartment. If no search
    /// parameters are listed, then the resource is not linked to the compartment.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Special Semantics
