// AUTO-GENERATED by atrius-fhir-generator (R6 RequestOrchestration)
use crate::r6::*;
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,instantiates_canonical,instantiates_uri,group_identifier,status,intent,priority,code")]
pub struct RequestOrchestration {
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **rqg-1**: Must have either resource, action, or definition (error)
    ///   Expression: `iif(resource.exists(), 1, 0) + iif(action.exists(), 1, 0) + iif(definition.exists(), 1, 0) = 1`
...
pub struct RequestOrchestrationAction {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **rqg-2**: RequestOrchestration related artifacts should only be documentation, justification, or citation (warning)
    ///   Expression: `type in ( 'documentation' | 'justification' | 'citation' )`
    ...
    /// detail, a Measure that specifies a measure to be evaluated, a
    /// MessageDefinition that specifies a message to be sent, a PlanDefinition that
    /// describes a series of actions to be taken, a Questionnaire that should be
    /// filled out, a SpecimenDefinition describing a specimen to be collected, an
