// AUTO-GENERATED by atrius-fhir-generator (R6 Medication)
use crate::r6::*;
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,code,status,marketing_authorization_holder,package_size")]
pub struct Medication {
    ...
    /// formulary code, optionally with translations to other code systems. The name
    /// of the medication can be conveyed in the code.text even if it is different
    /// from any of the coding displayName values.
    /// 
    /// ## Implementation Notes
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/medication-status|6.0.0-ballot3
    ...
    /// 
    /// ## Implementation Notes
    /// To represent a medication that has 5 vials with 10ml in each vial and the
    /// medication code just represents a vial, then the number of Items would be '5
    /// vial' and the amountPerItem would be '10ml/vial'. To represent a medication
    /// where there are 300 tablets and the code is for 100mg tablets, the number of
    /// items would be '300 tablets'
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **med-1**: Either numberOfItems or amountPerItem must exist (error)
    ///   Expression: `numberOfItems.exists() or amountPerItem.exists()`
    #[fhir_serde(rename = "packageSize")]
    pub package_size: Option<MedicationPackageSize>,
    ...
    /// specified. It is possible to specify both inactive and active ingredients. If
    /// Medication.code represents a fully specified product (the product + the
    /// strength + the dosage form) then including ingredients is duplicative.
    /// However, there may be implementation considerations which dictate including
    /// ingredients when Medication.code is a fully specified product. For compounded
    /// products, active and inactive ingredients should be included. However, even
    /// with all ingredients specified, a Medication resource should not be
    /// considered a "recipe" as the sequence and techniques for preparing the final
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    pub instance: Option<MedicationInstance>,
...
pub struct MedicationIngredient {
    ...
    /// 
    /// ## Implementation Notes
    /// The ingredient may reference a medication substance (for example,
    /// amoxicillin) or another medication (for example in the case of a compounded
    /// product, Glaxal Base). A referenced medication may be fully specified
    /// (amoxicillin 250 mg capsule), partially specified (amoxicillin 250mg), or may
    /// refer to an abstract ingredient (e.g., amoxicillin).
    /// 
    ...
    /// 
    /// ## Requirements
    /// This can be used in combination with the isActive element to provide further
    /// information about the type of ingredient.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// 
    /// ## Implementation Notes
    /// Medication.ingredient.strength.denominator can cause some confusion. For
    /// example, the concentration of a pediatric acetaminophen suspension can be '32
    /// mg / 1 ml' or '160 mg / 5 ml'. Both strengths are the same (32 mg * 5 = 160
    /// mg). In clinical systems (drug databases) strengths are usually to unit
    /// volumes (32 mg / 1 ml). While on a consumer product it's preferred to say how
    /// much per 'typical' dose (160 mg / 5 ml, with 5 ml (1 tsp) being the typical
    /// dose). Both representations are valid. Best practice is to select the
    /// denominator most relevant to the situation and use that same denominator for
    /// all ingredients. Ingredient.strength is problematic for multi-ingredient
    /// ingredients. For example, antacid products often contain multiple
    /// ingredients, such as 'aluminum & magnesium hydroxide with simethicone
    /// 400-400-40 mg/5ml'. Expressing a 'final concentration' in these cases is
    /// intractable and the best option is to not include ingredient.strength.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
...
///   Expression: `numberOfItems.exists() or amountPerItem.exists()`
#[derive(Debug, Clone, PartialEq, FhirSerde, FhirPath, Default)]
pub struct MedicationPackageSize {
    /// Unique id for inter-element referencing
    /// 
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_serde(rename = "numberOfItems")]
    pub number_of_items: Option<Quantity>,
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_serde(rename = "amountPerItem")]
    pub amount_per_item: Option<Ratio>,
}
