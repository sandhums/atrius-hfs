// @generated by helios-fhir-gen (R6 ValueSet) — do not edit
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,identifier,version,version_algorithm,name,title,status,experimental,date,publisher,contact,use_context,jurisdiction,immutable,effective_period")]
pub struct ValueSet {
    ...
    /// alphanumeric type name to ensure that it is machine-processing friendly.A
    /// name should be provided unless the value set is a contained resource (e.g. an
    ...
    /// 
    /// ## Implementation Notes
    /// Allows filtering of value sets that are appropriate for use versus not.See
    /// also the [Workflow Status
    /// Description](https://build.fhir.org/ig/HL7/fhir-extensions/StructureDefinition-valueset-workflowStatusDescription.html)
    ...
    /// that would be used for a production-level value set.
    /// 
    /// ## Implementation Notes
    ...
    /// 
    /// ## Implementation Notes
    /// Note that this is not the same as the meta.lastUpdated which is specific to
    /// an instance of a value set resource on a server. Additional specific dates
    /// may be added as extensions or be found by consulting Provenances associated
    ...
    /// 
    /// ## Requirements
    /// Helps establish the "authority/credibility" of the value set. May also allow
    /// for contact.
    /// 
    /// ## Implementation Notes
    /// Usually an organization but may be an individual. This is not necessarily the
    ...
    /// issues with the value set. This item SHOULD be populated unless the
    ...
    /// 
    /// ## Implementation Notes
    /// Description SHOULD contain instructions for clinical or administrative use
    /// and interpretation and information about misuse. Description SHOULD be
    /// populated unless the information is available from context. Description
    /// SHOULD be populated unless the value set is a contained resource (e.g. a
    /// value set defined solely in the context of a profile).
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// 
    /// ## Implementation Notes
    /// Normally immutability is set to 'false', which is the default assumption if
    /// it is not populated. Note that the implication is that if this is set to
    /// 'true', there may be only one ValueSet version for this definition.
    /// Immutability tends to be set to 'true' in one of two cases: - Where the value
    /// set, by the nature of its usage, cannot change. For example "All
    /// specializations of ACT in ActClassCode" - Where there's no safe way to
    /// express the "Purpose" such that someone else could safely make changes to the
    /// value set definition. Source workflow control must guarantee that the same
    /// URI always yields the same definition.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    /// 
    /// ## Implementation Notes
    /// This element does not describe the usage of the value set. Instead, it
    ...
    /// specifications that drove the structure of this value set.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// include restrictions on the use and publishing of the value set.
    /// 
    /// ## Requirements
    /// Consumers must be able to determine any legal restrictions on the use of the
    /// value set and/or its content.
    /// 
    /// ## Implementation Notes
    /// Frequently, the copyright differs between the value set and the codes that
    /// are included. The copyright statement should clearly differentiate between
    /// these when required.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// allows for a sunsetting process when new versions of the ValueSet are or are
    /// expected to be used instead.
    /// 
    /// ## Implementation Notes
    /// The effective period for a ValueSet determines when the content is applicable
    /// for usage and is independent of publication and review dates. For example, a
    /// valueset intended to be used for the year 2016 might be published in 2015.
    /// 
    /// See guidance around (not) making local changes to elements
    /// [here](canonicalresource.html#localization).
    /// 
    ...
    /// 
    /// ## Requirements
    /// Repositories must be able to determine how to categorize the ValueSet so that
    ...
    /// 
    /// ## Requirements
    /// A ValueSet must be able to provide enough information for consumers of the
    ...
    /// the content. If the type is `successor` or `predecessor`, the related
    /// resource must be a ValueSet.
    /// 
    /// ## Implementation Notes
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **vsd-12**: Related artifact can't be depends-on or composed-of (error)
    ...
    /// for data entry or validation. Value set expansions are always considered to
    /// be stateless - they are a record of the set of codes in the value set at a
    /// point in time under a given set of conditions, and are not subject to ongoing
    /// maintenance.
    /// 
...
pub struct ValueSetCompose {
    ...
    /// the value set may have different expansions as underlying code systems and/or
    /// value sets evolve. The interpretation of lockedDate is often dependent on the
    /// context - e.g. a SNOMED CT derived value set with a lockedDate will have a
    /// different expansion in USA than in UK. If a value set specifies a version for
    /// include and exclude statements, and also specifies a locked date, the
    /// specified versions need to be available that date, or the value set will not
    /// be usable.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// the expansion. If absent, the behavior is determined by the implementation,
    /// or by the applicable $expand parameters (but generally, inactive codes would
    ...
    /// inactive. Code systems should define what codes are considered to be
    /// inactive. If this is not clearly defined (including in the FHIR code system
    /// resource), then all codes are assumed to be active.
    /// 
    /// The Value Set Definition specification defines an ActiveOnly element, which
    /// is the reverse of this element e.g. (ValueSet.compose.inactive=FALSE) is the
    /// same as (VSD.ActiveOnly=TRUE).
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    /// codes from the system are listed. If one or more filters are listed, all of
    /// the filters must apply. If one or more value sets are listed, the codes must
    /// be in all the value sets. E.g. each include is 'include all the codes that
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **vsd-1**: A value set include/exclude SHALL have a value set or a system (error)
    ///   Expression: `valueSet.exists() or system.exists()`
    /// - **vsd-2**: A value set with concepts or filters SHALL include a system (error)
    ///   Expression: `(concept.exists() or filter.exists()) implies system.exists()`
    /// - **vsd-3**: Cannot have both concept and filter (error)
...
pub struct ValueSetComposeInclude {
    ...
    /// 
    /// ## Implementation Notes
    /// If there are no codes or filters, the entire code system is included. Note
    /// that the set of codes that are included may contain abstract codes. See
    /// ''Coding.system'' for further documentation about the correct value for the
    /// system element. If a contained code system is referenced, the entire
    /// canonical url must be provided.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    /// 
    /// ## Implementation Notes
    /// The list of concepts is considered ordered, though the order might not have
    ...
    /// 
    /// ## Implementation Notes
    /// Selecting codes by specifying filters based on properties is only possible
    /// where the underlying code system defines appropriate properties. Note that in
    /// some cases, the underlying code system defines the logical concepts but not
    /// the literal codes for the concepts. In such cases, the literal definitions
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **vsd-13**: A value set filter SHALL have either a value or sub-filters (error)
    ///   Expression: `value.exists() xor filter.exists()`
    /// 
    /// ## Conditions
    /// Used when: vsd-2, vsd-3
    ...
    /// 
    /// ## Implementation Notes
    /// The value set URI is either a logical reference to a defined value set such
    /// as a [SNOMED CT reference set](https://terminology.hl7.org/SNOMEDCT.html), or
    /// a direct reference to a value set definition using ValueSet.url. The
    /// reference might not refer to an actual FHIR ValueSet resource; in this case,
    /// whatever is referred to is an implicit definition of a value set that needs
...
pub struct ValueSetComposeIncludeConcept {
    ...
    /// when this concept is used in this particular value set. See notes in the
    /// value set narrative about the correct use of this element.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **vsd-11**: Must have a value for concept.designation.use if concept.designation.additionalUse is present (error)
...
pub struct ValueSetComposeIncludeFilter {
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    pub property: Code,
    /// = | is-a | descendent-of | is-not-a | regex | in | not-in | generalizes | child-of | descendent-leaf | exists | property-value-of
    /// 
    /// The kind of operation to perform as a part of the filter criteria.
    /// 
    /// ## Implementation Notes
    /// In case filter.property represents a property of the system, the operation
    /// applies to the selected property. In case filter.property represents a filter
    /// of the system, the operation SHALL match one of the
    /// CodeSystem.filter.operator values.
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    /// ## Special Semantics
    ...
    /// string value must be as described in [Using Filter
    /// Values](valueset.html#filter-values).
    /// 
    /// ## Implementation Notes
    ...
    /// 
    /// Nested filters which may be used if and only if the operator allows for a
    /// list of concept codes or a value set. This element supports defining the
    /// outer filter with a value, or by the application of one or more sub-filters.
    /// If one or more sub-filters are present, the intersection of the filters
    /// specifies the set of concept codes that are the `value` the operator acts on.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
...
pub struct ValueSetExpansion {
    ...
    /// 
    /// ## Implementation Notes
    /// Clients SHOULD use the next link, if present, to page through expansion
    /// results in preference to using the offset and count parameters. Due to the
    /// optional nature of the next link, its absence does not necessarily indicate
    /// that it is the last page of results. Instead, as the offset and count
    /// parameters SHALL be populated when paging, clients can reliably use the
    ...
    /// 
    /// ## Implementation Notes
    /// This SHOULD be a fully populated instant, but in some circumstances, value
    /// sets are expanded by hand, and the expansion is published without that
    /// precision.
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    ...
    /// more using the offset parameter. The `total` element SHOULD be present in an
    /// expansion. If the `total` element is not present, the expansion SHALL be
    /// treated as one page in a paged expansion.
    /// 
    /// ## Implementation Notes
    ...
    /// 
    /// ## Implementation Notes
    /// Paging only applies to flat expansions. If a filter is applied, the count is
    /// the number of concepts that matched the filter, not the number of concepts in
    /// an unfiltered view of the expansion.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    /// - When missing: Paging is not being used
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    ...
    /// 
    /// ## Implementation Notes
    /// The server decides which parameters to include here, but at a minimum, the
    ...
    /// If the codeSystem on the server has a specified version then this version
    /// SHALL be provided as a parameter in the expansion (note that not all code
    /// systems have a version).
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **vsd-6**: SHALL have a code or a display (error)
    ///   Expression: `code.exists() or display.exists()`
    /// - **vsd-9**: SHALL have a code if not abstract (error)
    ///   Expression: `code.exists() or abstract = true`
    /// - **vsd-10**: SHALL have a system if a code is present (error)
...
pub struct ValueSetExpansionContains {
    ...
    /// subsumption testing). The client should know whether it is appropriate for
    /// the user to select an abstract code or not.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    /// - When missing: If this is not populated, systems can assume that the concept it not abstract. This SHALL always be populated for abstract concepts
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    ...
    /// are those that are no longer to be used, but are maintained by the code
    /// system for understanding legacy data. It might not be known or specified
    /// whether a concept is inactive (and it may depend on the context of use).
    /// 
    /// ## Implementation Notes
    ...
    /// 
    /// ## Implementation Notes
    /// The exact value of the version string is specified by the system from which
    /// the code is derived.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
