// @generated by helios-fhir-gen (R6 TerminologyCapabilities) — do not edit
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,identifier,version,version_algorithm,name,title,status,experimental,date,publisher,contact,use_context,jurisdiction,copyright,kind,software,implementation,locked_date")]
pub struct TerminologyCapabilities {
    ...
    /// for testing purposes (or education/evaluation/marketing) and no version of
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    /// - **tcp-1**: If there is more than one version, a version value must be defined (error)
    ///   Expression: `version.count() > 1 implies version.all(value.exists())`
    /// - **tcp-7**: Each version.value element must be distinct for a particular code system. (error)
    ///   Expression: `version.value.isDistinct()`
...
/// Choice of types for the versionAlgorithm\[x\] field in TerminologyCapabilitiesCodeSystem
#[derive(Debug, Clone, PartialEq, FhirSerde, FhirPath)]
#[fhir_choice_element(base_name = "versionAlgorithm")]
pub enum TerminologyCapabilitiesCodeSystemVersionAlgorithm {
    /// Variant accepting the String type.
    #[fhir_serde(rename = "versionAlgorithmString")]
    String(String),
    /// Variant accepting the Coding type.
...
pub struct TerminologyCapabilitiesCodeSystem {
    ...
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    pub supplement: Option<Vec<Canonical>>,
    /// How to compare versions
    /// 
    ...
    /// 
    /// ## Implementation Notes
    /// This value indicates the version algorithm associated with all instances of
    /// the CodeSystem on the server. If the version algorithm is known to be
    /// different amongst different instances of the code system, then this element
    /// should not be used, and implementers should consider using the
    /// artifact-versionAlgorithm extension on the appropriate version descriptors
    /// instead.
    /// 
    /// If set as a string, this is a FHIRPath expression that has two additional
    /// context variables passed in - %version1 and %version2 and will return a
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/version-algorithm
    #[fhir_serde(flatten)]
    pub version_algorithm: Option<TerminologyCapabilitiesCodeSystemVersionAlgorithm>,
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/codesystem-content-mode|6.0.0
...
pub struct TerminologyCapabilitiesCodeSystemVersion {
    ...
    /// (and by implication, not all of the supported versions). This reference
...
pub struct TerminologyCapabilitiesImplementation {
    ...
    ///   Expression: `hasValue() or (children().count() > id.count())`
    pub url: Option<Url>,
    /// Describes how fragments are supported
    /// 
    /// This element provides a description of how fragments are supported (or not).
    /// 
    /// ## Implementation Notes
    /// Presence of the fragmentSupport element implies that the server supports
    /// fragments in some way. This element typically documents whether a server
    /// supports multiple fragments of the same code system, or whether an import
    /// facility (or similar) is expected to synthesize a single combined code system
    /// for internal use; whether fragments are disjoint, may overlap, or may be
    /// subsets of each other; and clarify behavior when multiple fragments or a
    /// fragment or example code system contain differing information for the same
    /// concept.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    /// - Included in summary
    /// - When missing: If this is not present, the server can be assumed to not support fragments
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_serde(rename = "fragmentSupport")]
    pub fragment_support: Option<Markdown>,
    /// Describes how supplements are supported
    /// 
    /// This element provides a description of how supplements are supported (or
    /// not).
    /// 
    /// ## Implementation Notes
    /// Presence of the supplementSupport element implies that the server supports
    /// supplements in some way. This element typically documents whether a server
    /// supports code system supplements of complete code systems and/or fragments.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    /// - Included in summary
    /// - When missing: If this is not present, the server can be assumed to not support supplements
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    #[fhir_serde(rename = "supplementSupport")]
    pub supplement_support: Option<Markdown>,
}
...
pub struct TerminologyCapabilitiesSupplements {
    ...
    /// 
    /// Codes that describes how the server implements global language packs - that
    /// is, supplements that contain only designations for a single language that are
    /// marked as intended by the extension
    /// http://hl7.org/fhir/StructureDefinition/codesystem-globalLangPack for use
    /// with all value sets and codesystem without explicit reference.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
