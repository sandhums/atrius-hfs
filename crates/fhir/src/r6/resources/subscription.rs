// AUTO-GENERATED by atrius-fhir-generator (R6 Subscription)
use crate::r6::*;
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,identifier,name,status,topic,contact,end,managing_entity,reason,filter_by,channel_type,endpoint,heartbeat_period,timeout,content_type,content,max_count")]
pub struct Subscription {
    ...
    /// 
    /// ## Implementation Notes
    /// Note that the managing entity for a subscription does not need to be the
    /// client (e.g., in the case of server-managed Subscriptions).
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    /// 
    /// ## Implementation Notes
    /// The URL requirements are based on channel type. For example, in a `rest-hook`
    /// channel, the URL must use the `http` or `https` protocol and be supplied by
    /// the client; in a `websocket` channel, the URL uses the `ws` or `wss` protocol
    /// and is supplied by the server.
    /// 
    /// Note that the URI is allowed to be relative. Relative URLs are relative to
    /// the FHIR server base URL (since there may be more than one, clients should
    ...
    /// 
    /// ## Implementation Notes
    /// Exactly what these mean depends on the channel type. They can convey
    ...
    /// 
    /// How much resource content to deliver in the notification payloads. The
    ...
    /// responsible for ensuring that the content is appropriately secured. For
    /// additional information, see [Notification
    /// Content](subscription.html#payloads).
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
...
pub struct SubscriptionFilterBy {
    ...
    /// multiple event triggers, this can be used to specify a single one. Multiple
    /// values are or-joined, multiple codings within a value are and-joined.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Special Semantics
