// AUTO-GENERATED by atrius-fhir-generator (R6 MedicationDispense)
use crate::r6::*;
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,modifier_extension,status,medication,subject,when_prepared")]
pub struct MedicationDispense {
    ...
    /// preparation | in-progress | cancelled | on-hold | completed | entered-in-error | unfulfilled | declined | unknown
    /// 
    ...
    /// - **ValueSet**: http://hl7.org/fhir/ValueSet/medicationdispense-status|6.0.0-ballot3
    pub status: Code,
    ...
    /// partial fill of 40 tablets (full quantity is 100 tablets) adds 1 to the prior
    /// fill number, not 0.4. This means that the 'fillNumber' may sometimes be
    /// higher than the MedicationRequest.numberOfRepeatsAllowed, as the latter
    /// assumes full fills, while fillNumber does not.
    /// 
    /// ## Implementation Notes
    /// This element may be inaccurate as it relies on the dispensing pharmacy to be
    /// aware of all fills that have occurred for the request.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Constraints
    ...
    /// ## Binding
    /// - **Strength**: example
    /// - **Description**: DoseAdministrationAid
...
pub struct MedicationDispensePerformer {
    ...
    /// 
    /// ## Implementation Notes
    /// The different performer functions within a dispense process can vary - and
    /// some of these functions may be restricted to clinical professionals - such
    /// distinction is usually specific to a jurisdiction and context. While
    /// performer.function and performer.actor don't add restrictions on which
    /// resources can be referenced, this element is expected to be used according to
    /// the local restrictions, for example not allowing Group (e.g., household) or
    /// RelatedPerson to be the dispenser.
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    /// ## Constraints
