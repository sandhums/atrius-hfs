// @generated by helios-fhir-gen (R6 CodeSystem) — do not edit
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,identifier,version,version_algorithm,name,title,status,experimental,date,publisher,contact,use_context,jurisdiction,effective_period,case_sensitive,value_set,hierarchy_meaning,compositional,version_needed,content,supplements,count,filter,property")]
pub struct CodeSystem {
    ...
    /// 
    /// ## Constraints
    /// - **csd-8**: Related artifact can't be depends-on or composed-of (error)
    ...
    /// case sensitive or not. When case sensitivity is not known, systems SHOULD
    /// produce codes with the correct case, and treat codes as case-sensitive, since
    /// this will mean any incorrect interpretation of codes is not a silent failure.
    /// This element is primarily provided to support validation software.
    /// Case-insensitive comparisons do not take locale into account, and will result
    /// in unsatisfactory results for certain locales. Character case definitions and
    /// conversions are out of scope for the FHIR standard, and the results of such
    ...
    /// grammar is, only whether or not there is one. Note that the code system
    /// resource does not define what the compositional grammar is, only whether or
    /// not there is one. Review the Using Code System X with HL7 Standards (where
    /// Code System X is a specific code system) pages at
    /// [terminology.hl7.org](https://terminology.hl7.org/external_code_systems.html)
    /// to learn about a code system's post coordination support.
    /// [Coding.code](datatypes-definitions.html#Coding.code) has some more
    /// information about how a post coordinated expression may be exchanged in a
    /// code element.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// 
    /// ## Constraints
    /// - **csd-6**: If a concept identifies an alternate, there must be a reciprocal relationship (error)
    ///   Expression: `defineVariable('sc', code).property.all((code = 'alternateCode') implies defineVariable('ac', value).%resource.repeat(concept).where(code = %ac).exists(property.where(code = 'alternateCode').value = %sc))`
    /// - **ele-1**: All FHIR elements must have a @value or children or both (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
...
pub struct CodeSystemConcept {
    ...
    /// ```designation.use``` of "primary designation" and a language equal to the
...
pub struct CodeSystemConceptProperty {
    ...
    /// 
    /// Identifies a property that is either defined in this CodeSystem
    /// (`CodeSystem.property.code`) or is a code from
    /// [http://hl7.org/fhir/concept-properties](codesystem.html#defined-props).
    /// 
    /// ## Implementation Notes
    /// Note that when the `CodeSystem.property.code` is defined in the CodeSystem
    /// itself, there is no mechanism to create a ValueSet, so there is no
    /// terminology binding.
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
...
pub struct CodeSystemFilter {
    ...
    /// 
    /// CodeSystem.filter.value is represented as a string, and the string value must
    /// be as described in [Using Filter Values](valueset.html#filter-values).
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    /// ## Special Semantics
...
pub struct CodeSystemProperty {
    ...
    /// 
    /// ## Implementation Notes
    /// The URIs used here SHOULD be drawn from the [Concept
    /// Properties](codesystem-concept-properties.html) code system, where the URI
    /// string consists of \[code system URL]#\[code]
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    /// defined by the code system or another code system.
    /// 
    /// ## Implementation Notes
    /// If type is code, and a defined set of codes is to be used, these codes can be
    /// specified by using the extension
