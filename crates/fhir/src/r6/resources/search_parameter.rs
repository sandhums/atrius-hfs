// @generated by helios-fhir-gen (R6 SearchParameter) — do not edit
...
#[fhir_resource(choice_elements = "versionAlgorithm", summary_fields = "id,meta,implicit_rules,modifier_extension,url,identifier,version,version_algorithm,name,title,status,experimental,date,publisher,contact,description,use_context,jurisdiction,code,alias_code,base,r#type")]
pub struct SearchParameter {
    ...
    /// address at which an authoritative instance of this search parameter is (or
    ...
    /// search parameter with the format [url]|[version]. The version SHOULD NOT
    ...
    /// but detail how it is supported by the server. This canonical reference SHOULD
    /// always be version-specific, as different versions of the base resource may
    /// have changes that would cause the derived search parameter to not be a valid
    /// derivation.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// the search parameter. Additional specific dates may be added as extensions or
    ...
    /// search parameter as conveyed in the 'text' field of the resource itself. This
    /// item SHOULD be populated unless the information is available from context
    /// (e.g. the language of the search parameter is presumed to be the predominant
    /// language in the place the search parameter was created).
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    /// ## Special Semantics
    ...
    /// search parameter and/or its content.
    /// 
    /// ## Implementation Notes
    /// ...
    /// 
    ...
    /// 
    /// The label that is recommended to be used in the URL or the parameter name in
    /// a parameters resource for this search parameter. In some cases, servers may
    /// need to use a different CapabilityStatement searchParam.name to differentiate
    /// between multiple SearchParameters that happen to have the same code.
    /// 
    /// ## Implementation Notes
    /// For maximum compatibility, use only lowercase ASCII characters and hyphens.
    /// The characters '.', '$', '|' and whitespace are prohibited as they are used
    /// as separators in search parameter processing. Note that HL7 will never define
    ...
    /// provide backwards-compatibility for renamed search parameters and
    /// translations into localized languages.
    /// 
    /// ## Implementation Notes
    /// For maximum compatibility, use only lowercase ASCII characters and hyphens.
    /// 
    ...
    /// This must be a resource, or a type with a [Structure Type Characterstics
    /// (Constraint
    ...
    /// 
    /// ## Implementation Notes
    /// If pre-adopting a new type from a future FHIR version, use the 'special' code
    /// and communicate the alternate type as an extension.
    /// 
    /// ## Cardinality: Required (1..1)
    /// 
    /// ## Special Semantics
    ...
    /// 
    /// Types of resource (if a resource is referenced). This must be a resource, or
    /// a type with a [Structure Type Characterstics (Constraint
    /// Control)](https://build.fhir.org/ig/HL7/fhir-extensions/StructureDefinition-structuredefinition-type-characteristics.html)
    /// of [can-be-target (Can Be
    /// Target)](https://build.fhir.org/ig/HL7/fhir-extensions/CodeSystem-type-characteristics-code.html#type-characteristics-code-can-be-target).
    /// 
    /// ## Implementation Notes
    /// In some cases, targets may exist for a search parameter that do not exist for
    /// the specified FHIRPath for all of the resources identified in
    /// SearchParameter.base. For example, the core clinical-encounter search
    /// parameter allows both Encounter and EpisodeOfCare as targets, even though not
    /// all of the base resources have EpisodeOfCare as a valid target.
    /// 
    /// However, the list of targets SHOULD cover all targets that might appear that
    /// are permitted by the specified FHIRPath.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
    ...
    /// 
    /// ## Special Semantics
    /// - When missing: If not declared, then it is up to server discretion whether multiple 'or' repetitions are allowed
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    ...
    /// 
    /// ## Special Semantics
    /// - When missing: If not declared, then it is up to server discretion whether multiple 'and' repetitions are allowed
    /// 
    /// ## Constraints
    /// - **ele-1**: All FHIR elements must have a @value or children (error)
    ///   Expression: `hasValue() or (children().count() > id.count())`
    ...
    ///   at all, then no presumptions can be made about the degree of chaining
    ///   support available
    /// * A server may use a single chain value of '*' to mean that all reference
    ///   search types supported can be chained rather than enumerating all chain
    ///   values
    /// * If a particular reference search parameter is supported, but not subsequent
    ///   chaining, then just list that search parameter name - e.g. for
    ///   Patient.practitioner, the chain value would be ```practitioner```
    /// * If a particular reference search parameter is supported with unlimited
    ///   chaining beneath it, then follow the search parameter name with '.'. E.g.
    ///   ```practitioner.```
    /// * If there are specific (not unlimited) chaining paths allowed they can be
    ///   explicitly enumerated. E.g. ```practitioner.organization```
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Constraints
