// @generated by helios-fhir-gen (R4B ElementDefinition) — do not edit
...
pub struct ElementDefinition {
    ...
    /// 
    /// ## Constraints
    /// - **eld-1**: If there are no discriminators, there must be a definition (error)
    ...
    /// Quantity is interpreted as an canonical minimum - e.g. you cannot provide
    ...
    /// Quantity is interpreted as an canonical maximum - e.g. you cannot provide 10g
    ...
    /// 
    /// ## Implementation Notes
    /// "Something useful" is context dependent and impossible to describe in the
    /// base FHIR specification. For this reason, tue mustSupport flag is never set
    /// to true by the FHIR specification itself - it is only set to true in
    /// profiles. A profile on a type can always make musSupport = true if it is
    ...
    /// it cannot be changed in derived profiles. An element/extension that has
    /// isModifier=true SHOULD also have a minimum cardinality of 1, so that there is
    /// no lack of clarity about what to do if it is missing. If it can be missing,
    /// the definition SHALL make the meaning of a missing element clear.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
    ...
    /// so servers might not support per-profile use of the isSummary flag. When a
    /// request is made with _summary=true, serailisers only include elements marked
    /// as 'isSummary = true'. Other than Attachment.data, all data type properties
    /// are included in the summary form. In resource and data type definitions, if
    /// an element is at the root or has a parent that is 'mustSupport' and the
    /// minimum cardinality is 1 or the element is a modifier, it must be marked as
    /// isSummary=true.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    ...
    /// 
    /// ## Constraints
    /// - **eld-12**: ValueSet SHALL start with http:// or https:// or urn: (error)
...
pub struct ElementDefinitionConstraint {
    ...
    /// 
    /// ## Implementation Notes
    /// Elements SHALL use "f" as the namespace prefix for the FHIR namespace, and
    /// "x" for the xhtml namespace, and SHALL NOT use any other prefixes. Note:
    /// XPath is generally considered not useful because it does not apply to JSON
    /// and other formats and because of XSLT implementation issues, and may be
...
pub struct ElementDefinitionType {
    ...
    /// the [profile-element](extension-elementdefinition-profile-element.html)
    /// extension.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
