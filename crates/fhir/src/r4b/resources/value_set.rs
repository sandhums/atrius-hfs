// @generated by helios-fhir-gen (R4B ValueSet) — do not edit
...
#[fhir_resource(summary_fields = "id,meta,implicit_rules,url,identifier,version,name,title,status,experimental,date,publisher,contact,use_context,jurisdiction,immutable")]
pub struct ValueSet {
    ...
    /// also the [valueset-workflowStatus](extension-valueset-workflowstatus.html)
    ...
    /// resource may be a secondary representation of the value set. Additional
    ...
    /// of the value set is the organization or individual primarily responsible for
    /// the maintenance and upkeep of the value set. This is not necessarily the same
    ...
    /// the value set. This item SHOULD be populated unless the information is
    ...
    /// is not a rendering of the value set as conveyed in the 'text' field of the
    /// resource itself. This item SHOULD be populated unless the information is
    /// available from context (e.g. the language of the value set is presumed to be
    /// the predominant language in the place the value set was created).The
    /// description is not intended to describe the semantics of the Value Set -
    /// there are no intrinsic semantics separate from the codes contained in its
    /// expansion. The description should capture its intended use, which is needed
    /// for ensuring integrity for its use in models across future changes. A
    /// description should be provided unless the value set is a contained resource
    /// (e.g. an anonymous value set in a profile). Most registries will require a
    /// description.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
...
pub struct ValueSetComposeInclude {
    ...
    /// as a [SNOMED CT reference set](snomedct.html#implicit), or a direct reference
    /// to a value set definition using ValueSet.url. The reference might not refer
    /// to an actual FHIR ValueSet resource; in this case, whatever is referred to is
    /// an implicit definition of a value set that needs to be clear about how
...
pub struct ValueSetExpansion {
    ...
    /// - **vsd-9**: Must have a code if not abstract (error)
    ///   Expression: `code.exists() or abstract = true`
    /// - **vsd-10**: Must have a system if a code is present (error)
