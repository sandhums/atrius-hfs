// @generated by helios-fhir-gen (R4B MedicationStatement) — do not edit
...
#[fhir_resource(choice_elements = "medication,effective", summary_fields = "id,meta,implicit_rules,identifier,based_on,part_of,status,category,medication,subject,context,effective,date_asserted")]
pub struct MedicationStatement {
    ...
    /// 
    /// ## Requirements
    /// This should not be used when indicating which resource a MedicationStatement
    /// has been derived from. If that is the use case, then
    /// MedicationStatement.derivedFrom should be used.
    /// 
    /// ## Cardinality: Optional, Multiple (0..*)
    /// 
    /// ## Special Semantics
    ...
    /// MedicationStatement.status contains codes that assert the status of the use
    /// of the medication by the patient (for example, stopped or on hold) as well as
    /// codes that assert the status of the medication statement itself (for example,
    /// entered in error).
    /// 
    ...
    /// Statements. If the medication is still being taken at the time the statement
    /// is recorded, the "end" date will be omitted. The date/time attribute supports
    /// a variety of dates - year, year/month and exact date. If something more than
    /// this is required, this should be conveyed as text.
    /// 
    /// ## Cardinality: Optional (0..1)
    /// 
    /// ## Special Semantics
