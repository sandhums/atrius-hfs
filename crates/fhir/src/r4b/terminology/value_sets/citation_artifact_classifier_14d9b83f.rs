...
impl CitationArtifactClassifier {
    ...
    pub const MODULE: &'static str = "citation_artifact_classifier_14d9b83f";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/citation-artifact-classifier" {
            return Some(super::super::code_systems::CitationArtifactClassifier::try_from_code(code).is_some());
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/citation-artifact-classifier" {
            return super::super::code_systems::CitationArtifactClassifier::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/citation-artifact-classifier".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/citation-artifact-classifier", code)
    }
