...
pub struct YesNoDonTKnow;

impl YesNoDonTKnow {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/yesnodontknow";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "yesnodontknow";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/data-absent-reason" {
            return Some(matches!(code, "asked-unknown"));
        }

        // Delegation to included ValueSets (compose.include.valueSet)
        let mut any_none = false;
        let mut any_false = false;
        // http://terminology.hl7.org/ValueSet/v2-0136 (not generated locally)
        any_none = true;
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/data-absent-reason".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/data-absent-reason", code)
    }
