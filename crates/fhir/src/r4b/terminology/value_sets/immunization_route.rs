...
pub struct ImmunizationRouteCodes;

impl ImmunizationRouteCodes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/immunization-route";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "immunization_route";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-RouteOfAdministration" {
            return Some(matches!(code, "IDINJ" | "IM" | "NASINHLC" | "IVINJ" | "PO" | "SQ" | "TRNSDERM"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-RouteOfAdministration" {
            return match code {
                "IDINJ" => Some("Injection, intradermal"),
                "IM" => Some("Injection, intramuscular"),
                "NASINHLC" => Some("Inhalation, nasal"),
                "IVINJ" => Some("Injection, intravenous"),
                "PO" => Some("Swallow, oral"),
                "SQ" => Some("Injection, subcutaneous"),
                "TRNSDERM" => Some("Transdermal"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-RouteOfAdministration".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-RouteOfAdministration", code)
    }
