...
impl LanguagePreferenceType {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/language-preference-type";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "language_preference_type_0f6def28";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/language-preference-type" {
            return Some(matches!(code, "verbal" | "written"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/language-preference-type".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/language-preference-type", code)
    }
