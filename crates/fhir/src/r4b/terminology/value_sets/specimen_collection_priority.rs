...
impl SpecimenCollectionPriority {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/specimen-collection-priority";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "specimen_collection_priority";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://example.com" {
            return Some(matches!(code, "1" | "2" | "3" | "4" | "5" | "6" | "7"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://example.com".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://example.com", code)
    }
