...
impl GenderIdentity {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/gender-identity";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "gender_identity_cfa35e33";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://hl7.org/fhir/gender-identity" {
            return Some(matches!(code, "male" | "female" | "non-binary" | "transgender-male" | "transgender-female" | "other" | "non-disclose"));
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://hl7.org/fhir/gender-identity" {
            return match code {
                ...
                "transgender-male" => Some("transgender male"),
                "transgender-female" => Some("transgender female"),
                "other" => Some("other"),
                "non-disclose" => Some("does not wish to disclose"),
                _ => None,
            };
        }
        None
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://hl7.org/fhir/gender-identity".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://hl7.org/fhir/gender-identity", code)
    }
