...
pub struct ExampleHierarchicalValueSet;

impl ExampleHierarchicalValueSet {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/example-hierarchical";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "example_hierarchical";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "#hacked" {
            return Some(matches!(code, "invalid" | "structure" | "required" | "value" | "processing" | "duplicate" | "not-found" | "conflict" | "lock" | "exception" | "tbrottled" | "login" | "unknown"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("#hacked".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
