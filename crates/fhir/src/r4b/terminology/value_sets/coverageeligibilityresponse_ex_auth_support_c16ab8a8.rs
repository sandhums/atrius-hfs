...
pub struct CoverageEligibilityResponseAuthSupportCodes;

impl CoverageEligibilityResponseAuthSupportCodes {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/coverageeligibilityresponse-ex-auth-support";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "coverageeligibilityresponse_ex_auth_support_c16ab8a8";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/coverageeligibilityresponse-ex-auth-support" {
            return Some(super::super::code_systems::CoverageEligibilityResponseAuthSupportCodes::try_from_code(code).is_some());
        }
        None
    }
    ...
    pub fn expected_display(system: &str, code: &str) -> Option<&'static str> {
        if system == "http://terminology.hl7.org/CodeSystem/coverageeligibilityresponse-ex-auth-support" {
            return super::super::code_systems::CoverageEligibilityResponseAuthSupportCodes::try_from_code(code).and_then(|c| c.display());
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/coverageeligibilityresponse-ex-auth-support".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/coverageeligibilityresponse-ex-auth-support", code)
    }
