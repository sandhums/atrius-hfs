...
pub struct MediaTypeCode;

impl MediaTypeCode {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/dicm-405-mediatype";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "dicm_405_mediatype";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://dicom.nema.org/resources/ontology/DCM" {
            return Some(matches!(code, "110030" | "110031" | "110032" | "110033" | "110034" | "110035" | "110036" | "110037" | "110010" | "110038"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://dicom.nema.org/resources/ontology/DCM".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://dicom.nema.org/resources/ontology/DCM", code)
    }
