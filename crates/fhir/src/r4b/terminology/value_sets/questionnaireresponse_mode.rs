...
impl QuestionnaireResponseMode {
    pub const URL: &'static str = "http://hl7.org/fhir/ValueSet/questionnaireresponse-mode";
    pub const VERSION: &'static str = "4.3.0";
    pub const MODULE: &'static str = "questionnaireresponse_mode";
    ...
    pub fn contains(system: &str, code: &str) -> Option<bool> {
        if system == "http://terminology.hl7.org/CodeSystem/v3-ParticipationMode" {
            return Some(matches!(code, "ELECTRONIC" | "VERBAL" | "WRITTEN"));
        }
        None
    }
    ...
    pub fn validate_code(code: &str) -> Result<(), TerminologyValidationError> {
        match Self::contains_implicit_code(code) {
            Some(true) => Ok(()),
            Some(false) => {
                Err(TerminologyValidationError::NotInValueSet { valueset_url: Self::URL.to_string(), system: Some("http://terminology.hl7.org/CodeSystem/v3-ParticipationMode".to_string()), code: code.to_string() })
            }
            None => Err(TerminologyValidationError::RemoteValidationRequired("Remote terminology validation required".to_string())),
        }
    }
    ...
    pub fn contains_implicit_code(code: &str) -> Option<bool> {
        Self::contains("http://terminology.hl7.org/CodeSystem/v3-ParticipationMode", code)
    }
