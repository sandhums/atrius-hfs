...
async fn test_chained_terminology_modifier_is_checked_against_the_terminal_type() {
    ...
    for (resource_type, key, value, expected) in [
        ...
        assert_eq!(ids(&response.json()), [expected], "{key}");
        let after = requests.lock().unwrap().len();
        // `$expand` is cached process-wide for a ValueSet URL (`:in`). A later
        // search for the same URL adds 0 calls. Atrius pages `$expand`, so a
        // cache miss may add two requests when `expansion.total` exceeds the
        // first page. Helios asserts exactly one call per search.
        assert!(
