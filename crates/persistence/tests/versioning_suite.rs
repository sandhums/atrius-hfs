//! Test-binary entry point that wires the `tests/versioning/` suite into the build.
//!
...
//! declares `mod <dir>;`. Without this file the entire `versioning/` suite is dead
//! code that never compiles or runs (issue #306). `mod versioning;` resolves to
//! `tests/versioning/mod.rs`.
//!
