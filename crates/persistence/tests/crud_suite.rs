//! Test-binary entry point that wires the `tests/crud/` suite into the build.
//!
...
//! declares `mod <dir>;`. Without this file the entire `crud/` suite is dead
//! code that never compiles or runs (issue #306). `mod crud;` resolves to
//! `tests/crud/mod.rs`.
//!
