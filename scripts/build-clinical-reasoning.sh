...
#   ./scripts/build-clinical-reasoning.sh --skip-hts   # HFS + bridge + cds only
#
...
echo
echo "Built:"
ls -la target/release/hts target/release/hfs target/release/cr-fhir-bridge target/release/cds-server 2>/dev/null || true
echo
echo "Next (separate terminals):"
echo "  ./scripts/run-hts.sh"
echo "  ./scripts/run-hfs.sh"
echo "  ./scripts/run-kr-hfs.sh"
echo "  ./scripts/run-cr-fhir-bridge.sh"
