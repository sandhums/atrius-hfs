...
#
# Simulates a CDS client: fetches Patient + chart prefetch from cr-fhir-bridge,
# POSTs encounter-start invoke to cds-server for service er-chest-pain-pathway.
#
# Prerequisites: HTS, clinical HFS, bridge, KR (PlanDefinition + Library + ADs),
...
for arg in "$@"; do
  case "$arg" in
    ...
    --bridge-apply) BRIDGE_APPLY=true ;;
  esac
done
...
fetch_json() {
  ...
  if ! curl -sfS -H 'Accept: application/fhir+json' "$url"; then
    ...
    echo "hint: start cr-fhir-bridge + clinical HFS; import ER demo patient data if needed" >&2
    exit 1
  fi
}
...
echo "Fetching ER chest pain prefetch from $bridge for patient $PATIENT (encounter-start) ..." >&2
...
payload="$(
  jq -n \
    ...
    --argjson encounter1 "$(fetch_json "$bridge/Encounter?patient=$PATIENT" "Encounter search")" \
    --argjson condition2 "$(fetch_json "$bridge/Condition?patient=$PATIENT" "Condition search")" \
    --argjson observation3 "$(fetch_json "$bridge/Observation?patient=$PATIENT" "Observation search")" \
    --argjson questionnaireresponse4 "$(fetch_json_optional "$bridge/QuestionnaireResponse?patient=$PATIENT" "QuestionnaireResponse search")" \
    --argjson servicerequest5 "$(fetch_json_optional "$bridge/ServiceRequest?patient=$PATIENT" "ServiceRequest search")" \
    --argjson medicationrequest6 "$(fetch_json_optional "$bridge/MedicationRequest?patient=$PATIENT" "MedicationRequest search")" \
    --argjson diagnosticreport7 "$(fetch_json_optional "$bridge/DiagnosticReport?patient=$PATIENT" "DiagnosticReport search")" \
    --argjson medicationadministration8 "$(fetch_json_optional "$bridge/MedicationAdministration?patient=$PATIENT" "MedicationAdministration search")" \
    ...
    --argjson procedures "$(fetch_json_optional "$bridge/Procedure?patient=$PATIENT" "Procedure search")" \
    --argjson medicationRequests "$(fetch_json_optional "$bridge/MedicationRequest?patient=$PATIENT" "MedicationRequest search")" \
    --argjson immunizations "$(fetch_json_optional "$bridge/Immunization?patient=$PATIENT" "Immunization search")" \
    --argjson diagnosticReports "$(fetch_json_optional "$bridge/DiagnosticReport?patient=$PATIENT" "DiagnosticReport search")" \
    --argjson serviceRequests "$(fetch_json_optional "$bridge/ServiceRequest?patient=$PATIENT" "ServiceRequest search")" \
    --argjson allergies "$(fetch_json_optional "$bridge/AllergyIntolerance?patient=$PATIENT" "AllergyIntolerance search")" \
    --argjson coverage "$(fetch_json_optional "$bridge/Coverage?beneficiary=Patient/$PATIENT" "Coverage search")" \
...
if [[ "$BRIDGE_APPLY" == "true" ]]; then
  echo "Bridge FHIR PlanDefinition/\$apply ..." >&2
  apply_payload="$(
    jq -n \
      --arg subject "Patient/$PATIENT" \
      --arg encounter "Encounter/$ENCOUNTER" \
      --arg practitioner "$PRACTITIONER" \
      '{
  ...
  apply_tmp="$(mktemp)"
  apply_code="$(
    curl -sS -o "$apply_tmp" -w '%{http_code}' \
      -X POST "$bridge/PlanDefinition/$PLAN_DEFINITION_ID/\$apply" \
      -H 'Content-Type: application/fhir+json' \
      -d "$apply_payload"
  )"
  if [[ "$apply_code" != "200" ]]; then
    echo "bridge \$apply HTTP $apply_code:" >&2
    cat "$apply_tmp" >&2
    exit 1
  fi
  jq '.parameter[] | select(.name=="return") | .resource | {resourceType, id, status}' "$apply_tmp"
  echo "ok: bridge PlanDefinition/\$apply returned CarePlan" >&2
  rm -f "$apply_tmp"
fi
