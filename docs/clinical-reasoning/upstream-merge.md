...
17 Sep 2026 (follow-on): `main` `fed93aab2` (Helios `25a53bd0d` / `#1211`) → `feat-clinical-reasoning`. Rollback tag `pre-merge-main-2026-09-17b` → `6f73317e1`. Ledger stamp SQLite 34 / Postgres 43. Helios SQLite v32 / Postgres v39 as named step `bulk_manifest_file_progress` (`#1127`) before dead-letter. Soft-delete search DELETE keeps `tx.execute` and takes the `#1197` tenant/type prefix. Bulk-submit keeps ingest validator and takes `#1127` `batch_size`/`skip_unchanged`; validation failures return the 3-tuple. Owner-scoped export-job tests kept with Helios `BackendError`. No fhir/auth/observability inbound (HIS pins unchanged). Bundle PATCH and write-observer `enqueue_resource_event` unchanged. Prior: 17 Sep morning `447a0b068`, tag `pre-merge-main-2026-09-17`.
