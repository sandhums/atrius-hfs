...
| `crates/persistence/src/backends/sqlite/schema.rs` | Keep named `SQLITE_STEPS`. Insert new Helios migrate fns **before** `OUTBOX_DEAD_LETTER_STEP`; bump `SCHEMA_VERSION` to `len+1`. Discard Helios integer `migrate_schema`. Helios v29 is `search_index_drop_token_display` (`#945`); v30 is `bulk_manifests_index_pending` (`#1125`); v31 is `search_index_resource_key` (`#945`). Fresh init runs the named steps so `create_indexes` may still CREATE `idx_search_token_display` / composite-on-`resource_id` and later named steps drop/rekey them. Integer stamp collision (feat tip vs Helios tip both 31) is expected — names distinguish |
| `crates/persistence/src/backends/sqlite/storage.rs` | Take Helios `resource_key` search-index DELETE SQL; keep feat **`tx.execute`**, not Helios `conn.execute`, so the delete and `maybe_enqueue_on_conn` share one transaction. Purge-after-row-delete still uses `resource_id` (Helios does too — the subquery would miss) |
...
17 Sep 2026: `main` `447a0b068` (Helios `8039e7b75`) → `feat-clinical-reasoning`. Rollback tag `pre-merge-main-2026-09-17` → `d87450858`. Ledger stamp SQLite 33 / Postgres 42. Helios SQLite 29→31 as named steps `bulk_manifests_index_pending` (`#1125`) and `search_index_resource_key` (`#945`) before dead-letter. Patient `$everything` `#966`. Mongo search_index generation 2 `#1059`/`#1084`. Soft-delete search cleanup uses `resource_key` on `tx`. `helios-observability` dashboard.rs inbound (HIS pin note; no fhir/auth). Bundle PATCH and write-observer `enqueue_resource_event` unchanged. Prior: 16 Sep `c878706ec`, tag `pre-merge-main-2026-09-16`.
