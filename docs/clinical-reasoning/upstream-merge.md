...
| `crates/persistence/src/backends/sqlite/schema.rs` | Keep named `SQLITE_STEPS`. Insert new Helios migrate fns **before** `OUTBOX_DEAD_LETTER_STEP`; bump `SCHEMA_VERSION` to `len+1`. Discard Helios integer `migrate_schema`. Helios v29 is `search_index_drop_token_display` (`#945`); fresh init runs the named steps so `create_indexes` may still CREATE `idx_search_token_display` and the drop step removes it |
...
16 Sep 2026: `main` `c878706ec` (Helios `46fc4cad1`) → `feat-clinical-reasoning`. Rollback tag `pre-merge-main-2026-09-16` → `793e1d80c`. Ledger stamp SQLite 31 / Postgres 42. Helios SQLite 28→29 (`idx_search_token_display` drop, `#945`) as named step `search_index_drop_token_display` before dead-letter. Number/quantity exact comparators `#1148`/`#1011`. Unknown type 404 `#989`. `hfs --version` `#992`. Bundle.total 0 `#990`. Cursor paging `#1079`. WAL `synchronous=NORMAL` `#947`. Mongo distinct-too-big `#999`. HIS pins unchanged (no inbound `helios-fhir` / `helios-auth` / `helios-observability`). Bundle PATCH and write-observer `enqueue_resource_event` unchanged. Prior: 15 Sep `9665cd848`, tag `pre-merge-main-2026-09-15`.
