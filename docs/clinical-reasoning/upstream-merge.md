...
| `src/core/patch.rs` | Take Helios `#1406` `apply_patch` / `PatchError` (FHIRPath is `UnsupportedFormat` → REST 501). Keep feat bundle helpers (`apply_resource_patch`, `patched_from_bundle_entry`, `patch_format_from_bundle_resource`). |

`SCHEMA_VERSION` (36 SQLite / 46 Postgres) is an operator stamp. Clinical restart after the ledger lands creates `schema_migrations` and backfills names; it must not replay the full Postgres index ladder. SQLite 20/21 are Helios `#903` `idx_resources_reindex` and `#944` partial family indexes (Helios numbered those v19/v20). SQLite 22–24 are Helios `#947`/`#967` (drop `idx_search_resource`, late partial indexes, `resource_fts_map`; Helios numbered those v21–v23). SQLite 25–26 are Helios `#959`/`#953` (`idx_resources_live_type`, manifest `phase`/`files_*`; Helios numbered those v24–v25). SQLite 27–28 are Helios manifest publication and export `types_*` (Helios numbered those v26–v27). SQLite 29 is Helios v28 (partial `idx_search_string_folded`). SQLite 30 is Helios v29 (`idx_search_token_display` drop, `#945`). SQLite 31 is Helios v30 (`bulk_manifests.index_pending`, `#1125`). SQLite 32 is Helios v31 (`search_index.resource_key`, `#945`). SQLite 33 is Helios v32 (`bulk_manifest_file_progress` / `skipped_entries`, `#1127`). SQLite 34 is Helios v33 (`bulk_export_jobs.attempts`, `#1041`). SQLite 35 is Helios v34 (`secondary_sync_failures`, `#1334`). `OUTBOX_DEAD_LETTER_STEP` is the SQLite tip (v36) so an upstream-numbered Helios v34 DB does not imply `dead_at` already applied (`implied_applied_indices` maps Helios v29 onto fork indices 16..=28, Helios v30 onto 16..=29, Helios v31 onto 16..=30, Helios v32 onto 16..=31, Helios v33 onto 16..=32, and Helios v34 onto 16..=33). Postgres 37–38 are the same publication/`types_*` steps (Helios numbered those v37–v38), Postgres 39 is Helios v39 file-progress, Postgres 40 is Helios v40 attempts (`#1041`, already on feat as `bulk_export_jobs_attempts`), Postgres 41 is Helios v41 `secondary_sync_failures` (`#1334`), Postgres 42 is Helios v42 `idx_search_reference_target_id` (`#1414`), then fork-only slot-2, `bulk_manifests_phase_progress`, and `dead_at` (tip v46). Helios v41/v42 named steps must sit **immediately after** `bulk_export_jobs_attempts` and **before** fork-only slot-2/phase, otherwise Upstream implied-applied would mark slot-2 as Helios v41. `migrate_v41_to_v42` must **not** `set_schema_version(..., 42)` (would regress a feat DB already at 44). Do not restore hourly retry of exhausted outbox rows.
...
| `crates/persistence/src/backends/sqlite/schema.rs` | Keep named `SQLITE_STEPS`. Insert new Helios migrate fns **before** `OUTBOX_DEAD_LETTER_STEP`; bump `SCHEMA_VERSION` to `len+1`. Discard Helios integer `migrate_schema`. Helios v29 is `search_index_drop_token_display` (`#945`); v30 is `bulk_manifests_index_pending` (`#1125`); v31 is `search_index_resource_key` (`#945`); v32 is `bulk_manifest_file_progress` (`#1127`); v33 is `bulk_export_jobs_attempts` (`#1041`); v34 is `secondary_sync_failures` (`#1334`). Postgres Helios v39 is file-progress (feat already used `migrate_v38_to_v39` for dead-letter — rename the Helios fn); v40 is attempts (already on feat — do **not** duplicate); v41 is `secondary_sync_failures`; v42 is `search_index_reference_target_id` (`#1414`). Place v41/v42 **immediately after** `bulk_export_jobs_attempts` and **before** fork-only slot-2/phase. Fresh init runs the named steps so `create_indexes` may still CREATE `idx_search_token_display` / composite-on-`resource_id` and later named steps drop/rekey them. Integer stamp collision (feat tip vs Helios tip) is expected — names distinguish |
| `crates/persistence/src/core/patch.rs` | Take Helios `#1406` `apply_patch` (FHIRPath 501). Keep feat Bundle PATCH helpers. REST instance PATCH must call `helios_persistence::core::apply_patch`, not a local stub that no-op-writes |
