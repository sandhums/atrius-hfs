...
21 Sep 2026: `main` `c8d7e4ab1` (Helios `9ccef30c6` / 0.2.3) → `feat-clinical-reasoning`. Rollback tag `pre-merge-main-2026-09-21` → `7b666e306`. Ledger stamp unchanged SQLite 35 / Postgres 44 (no inbound schema steps). Workspace still 0.2.3. Conflicts: `git rm` Helios flat `r*.rs` (keep directories); regen for `#1357` `ViewDefinition.resourceDefinition` (signatures match `origin/main`); `batch.rs` keep feat JSON/merge Bundle PATCH (`["not-supported", "value", "required"]`) and take Helios `#1322` decode-once (drop local `normalize_criteria`); CI keep Atrius `HeliosSoftware` skip on `ci-extended.yml` / `inferno-bulk-data.yml` plus Helios `RUSTUP_TOOLCHAIN` and Inferno job name `v4.0.0-ballot`. Instance FHIRPath Patch stays Helios `501` (persistence stub would no-op-write `Patient.name[0].family`); JSON Patch in bundles unchanged. Helios `#1379` `:in` on `code` elements taken; Atrius `$expand` paging + process-wide cache kept (tests allow 0–2 HTS calls per search). SQLite token-OR `INDEX_OR_CHUNK` and HTS ICD keep (`fork-icd-import-expand-subsumes-2026-09-20.md` items 1–5) survived; inbound did not touch `crates/hts/`. HIS pins: `helios-fhir` models gained `resourceDefinition` — bump after this merge is pushed if HIS should lockstep. Prior: 20 Sep `ef6d1e7a7`, tag `pre-merge-main-2026-09-20`.
