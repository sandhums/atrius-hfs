...
20 Sep 2026: `main` `32f816c96` (Helios `acb115153` / 0.2.3) → `feat-clinical-reasoning`. Rollback tag `pre-merge-main-2026-09-20` → `139e6f9f3`. Ledger stamp unchanged SQLite 35 / Postgres 44 (no inbound schema steps). Workspace 0.2.3; keep `helios-terminology-client` and `helios-auth` `features = ["redis"]`. HTS ICD `$expand`/`$subsumes`/`/import?finalize`/`DELETE` 204 kept (`fork-icd-import-expand-subsumes-2026-09-20.md`); Helios `#1277` `error_chain` auto-merged with feat `rebuild_missing_closures`. Took `#1242` collapse of `INDEX_DURING_INGEST` into `HFS_BULK_SUBMIT_DEFER_INDEXING` (default still true). Bundle PATCH and write-observer `enqueue_resource_event` unchanged. HIS pins: crate version 0.2.3 only (no fhir/auth/observability source); bump after this merge is pushed if HIS should lockstep. Prior: 18 Sep `b93579edf`, tag `pre-merge-main-2026-09-18`.
