...
description: >
  ...
  RxNorm, ICD-10), and OperationOutcome error handling. Also use for general FHIR/HL7
  modeling questions in this Helios HFS workspace.
metadata:
  vendor: anthropics/healthcare
  source: https://github.com/anthropics/healthcare/tree/main/plugins/healthcare/skills/fhir-developer
  license: Anthropic terms of service (see VENDOR.md)
---
...
1. Prefer this skill for **generic FHIR R4** modeling, cardinality, coding systems, REST
   status codes, Bundles, and SMART scope patterns.
2. Prefer the repo skills under `.claude/skills/` for **HFS-specific** runtime behavior
   (`work-with-auth`, `work-with-hts`, `work-with-fhirpath`, `run-hfs-server`, etc.).
3. Default FHIR version here is **R4** (`FhirVersion::default()`). R4B/R5/R6 are feature-
   gated; do not invent a different default than the workspace conventions.
